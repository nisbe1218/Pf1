...
Pipeline :
  SVM brut -> Isotonique (10-fold CV) -> seuils valides (T1/T2)

Sorties :
  iso_calibrator.joblib
  seuils_classification.joblib   (T1=10% sensib>=90% | T2=29% Youden bootstrap)
...
# T1 = premier seuil ROC avec sensibilite >= 90% (8 deces manques / 94, soit 8.5%)
# T2 = Index de Youden bootstrappe (1000 iterations, mediane=28.9%, IC95=[10%-33.3%])
T1_FIXE = 0.10   # Faible  < 10%  (sensibilite=91.5% : 8.5% deces manques seulement)
T2_FIXE = 0.29   # Elevee  > 29%  (Youden bootstrappe : separation optimale Modere/Eleve)
...
print()
print("=" * 56)
print("  RESULTATS AVEC SEUILS VALIDES : T1=10% / T2=29%")
print("  T1 : sensibilite>=90% sur courbe ROC | T2 : Youden bootstrappe (n=1000)")
print("=" * 56)
...
# SVM production : ajuste sur l'ensemble des donnees
pipe_svm.fit(X, y)
joblib.dump(pipe_svm, f'{DOSSIER}/mortalite_svm.joblib')
...
joblib.dump({
    'T1':         T1,
    'T2':         T2,
    'T1_methode': 'Sensibilite>=90% sur courbe ROC (91.5% atteinte, 8.5% deces manques)',
    'T2_methode': 'Index de Youden bootstrappe (n=1000, mediane=28.9%, IC95=[10%-33.3%])',
    'auc_brut':   round(auc_brut, 4),
    'auc_cal':    round(auc_cal, 4),
}, f'{DOSSIER}/seuils_classification.joblib')

joblib.dump({
    'features': list(X.columns),
    'T1':       T1,
    'T2':       T2,
    'pipeline': 'SVM -> Isotonique -> seuils 10%/29% (Sens>=90% | Youden bootstrap)',
}, f'{DOSSIER}/mortalite_svm_features.joblib')

# Export patient-level — CSV + Excel
cols_export = FEATS_CLINIQUES + [
    'age_annees', 'Charlson', 'sodium_basal',
    'deces', 'delai_jusquau_deces_jours', 'deces_1an'
]
cols_export = [c for c in cols_export if c in df.columns]
df_export = df[cols_export].copy().reset_index(drop=True)
df_export.insert(0, 'ID_PATIENT', range(1, len(y)+1))
df_export['PROB_SVM_BRUT_%']     = (proba_svm * 100).round(1)
df_export['PROB_CALIBREE_ISO_%'] = (proba_cal * 100).round(1)
df_export['ZONE_RISQUE']         = zones
df_export['DECES_1AN_OBSERVE']   = y

# Nettoyage : convertir tous les types en natifs Python
for col in df_export.columns:
    df_export[col] = pd.to_numeric(df_export[col], errors='ignore')
df_export = df_export.where(pd.notnull(df_export), other=None)

# Sauvegarde CSV (toujours fiable)
df_export.to_csv(f'{DOSSIER}/Resultats_Patients_HD478.csv', index=False)

# Sauvegarde Excel via openpyxl avec types forcés
import openpyxl
...
for _, row in df_export.iterrows():
    ws.append([
        ...
        else float(round(v, 3)) if isinstance(v, (float, np.floating))
        else str(v)
        for v in row
    ])
wb.save(f'{DOSSIER}/Resultats_Patients_HD478.xlsx')

print()
print("=" * 56)
print("  FICHIERS SAUVEGARDES")
print("=" * 56)
print(f"  mortalite_svm.joblib")
print(f"  iso_calibrator.joblib")
print(f"  seuils_classification.joblib    T1={T1*100:.0f}%  T2={T2*100:.0f}%")
print(f"  mortalite_svm_features.joblib")
print(f"  Resultats_Patients_HD478.xlsx  ({len(y)} patients)")
print(f"  Resultats_Patients_HD478.csv   ({len(y)} patients)")
print()
