...
for fold, (tr, te) in enumerate(cv.split(proba_oof, y)):
    iso_fold = IsotonicRegression(out_of_range='clip')
    iso_fold.fit(proba_oof[tr], y.values[tr])
    calibrated_cv[te] = iso_fold.predict(proba_oof[te])

# Mortalité honnête par zone
T1, T2 = 0.10, 0.40
zone_labels = np.where(calibrated_cv < T1, 'Faible',
              np.where(calibrated_cv < T2, 'Modéré', 'Élevé'))

print("\n=== RÉSULTATS HONNÊTES (calibration croisée) ===")
...
iso_final = IsotonicRegression(out_of_range='clip')
...
print(f"\n=== SEUILS SVM BRUTS ===")
print(f"  Calibré 10% → SVM brut = {t1_raw:.4f}  (Faible / Modéré)")
print(f"  Calibré 40% → SVM brut = {t2_raw:.4f}  (Modéré / Élevé)")

# Vérification
calib_t1 = float(iso_final.predict([t1_raw])[0])
calib_t2 = float(iso_final.predict([t2_raw])[0])
print(f"\n  Vérification : iso({t1_raw:.4f}) = {calib_t1:.3f}  (cible 0.10)")
print(f"  Vérification : iso({t2_raw:.4f}) = {calib_t2:.3f}  (cible 0.40)")
...
m['calibration_thresholds'] = {'T1_clinical': T1, 'T2_clinical': T2,
                                't1_raw': t1_raw, 't2_raw': t2_raw}
m['iso_mortality_rates']    = {
    'Faible':  round(float(y[zone_labels == 'Faible'].mean() * 100), 1),
    'Modéré':  round(float(y[zone_labels == 'Modéré'].mean() * 100), 1),
    'Élevé':   round(float(y[zone_labels == 'Élevé'].mean()  * 100), 1),
}
