...
\begin{itemize}
  \item \textbf{Deterministic only}: clean file ($\leq$50 rows, 0\% missing, no outliers) — Python rules only, no LLM call, sub-second processing
  \item \textbf{Balanced}: moderate complexity — Qwen2.5:14B with reduced context window, faster inference
  \item \textbf{Advanced}: complex medical dataset ($>$300 rows, $\geq$10\% missing, or clinical complexity score $\geq$6) — Qwen2.5:14B with full RAG context and extended processing
\end{itemize}
