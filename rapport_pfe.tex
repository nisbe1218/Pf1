...
\begin{tabular}{p{0.46\linewidth}p{0.46\linewidth}}
  ...
  \textbf{Defended on:} 25~June~2026 &\textbf{Hosting Company:}\\[0.25cm]
                             & HAIM-FMPDF                 \\[0.7cm]
  \textbf{Jury Members:}    &                            \\[0.3cm]
  Prof.\ Nisrine Lachgar     &                            \\[0.15cm]
  Prof.\ Meryam El~Mouhtadi  &                            \\
\end{tabular}
...
Six machine learning algorithms were systematically evaluated using stratified 10-fold cross-validation. A \textbf{linear SVM} emerged as the best-performing model (AUC-ROC = 0.8272, PR-AUC = 0.484, F1 = 0.551, Brier Score = 0.126), combining strong discriminative ability with clinical interpretability. Raw SVM probabilities are calibrated via Platt scaling (\texttt{CalibratedClassifierCV}, 5-fold sigmoid) and mapped to three evidence-based clinical risk zones, with thresholds derived by ROC curve analysis (T1 = 10\%, Sensitivity $\geq$ 90\%) and bootstrapped Youden index (T2 = 29\%, $n=1000$ iterations, median = 28.9\%): \textbf{Low Risk} ($\hat{p} < 10\%$, observed mortality 3.8\%), \textbf{Moderate Risk} ($10\% \leq \hat{p} < 29\%$, 15.6\%), and \textbf{High Risk} ($\hat{p} \geq 29\%$, 46.5\%).
...
\chapteropening{1}{General Introduction}{Context, medical setting, and project objectives.}{%
  ...
  \textbf{1.7}\hspace{8pt}Report Organization \dotfill \pageref{sec:organization}\\[3pt]
}
...
\caption{Trello --- Project management tool used for task tracking throughout the project lifecycle}
\label{fig:trello}
\end{figure}

\section{Report Organization}\label{sec:organization}

We organize this report into seven chapters that follow the logical progression of our work, from the identification of the clinical problem to the deployment of a validated solution.

\textbf{Chapter~1 — General Introduction} sets the stage. We introduce the HAIM Center and the CHU Hassan II nephrology department, and we lay out the medical context of chronic kidney disease and hemodialysis. We then identify the three problems we observed at the outset of this project: years of clinical records siloed in Excel files with no exploitation, the complete absence of a locally validated ML-based mortality prediction tool, and severe data quality issues in the HD-478 cohort — up to 30\% missing values, inconsistent units, and free-text entries in numerical fields — that made any direct ML application impossible.

\textbf{Chapter~2 — State of the Art} reviews the existing scientific literature. We survey published CDSS for nephrology, ML approaches to dialysis mortality prediction, and feature engineering strategies for clinical tabular data. We note that while individual components exist in the literature, no prior work combines an LLM-assisted preprocessing pipeline, calibrated probabilistic output, and clinically grounded risk zones in a single deployable platform. This gap is what motivated the solution we propose.

\textbf{Chapter~3 — Global System Architecture} describes the overall design we adopted to address the identified problems. We propose a full-stack web platform organized around three modules: a patient data management layer, an AI preprocessing pipeline, and an ML prediction engine. We explain our technology choices (Django REST Framework, React, PostgreSQL, Docker Compose) and present the use case model covering the four user roles we designed around the clinical workflows at CHU Hassan II.

\textbf{Chapter~4 — Data Engineering Pipeline} details the first solution we implemented: an \textbf{LLM+RAG preprocessing pipeline} (Qwen2.5:14B + ChromaDB) that automatically detects and corrects the data quality issues we observed in the HD-478 cohort. We noticed that blindly applying a model's corrections would be clinically unacceptable, so we paired the pipeline with a supervised validation interface where clinicians review and approve each correction before integration. We also describe the 32-feature engineering strategy we developed, including 8 interaction features (albumin$\times$Charlson, Kt/V$\times$residual diuresis, etc.) whose clinical relevance we confirmed with the nephrology team.

\textbf{Chapter~5 — Machine Learning Layer} presents the second core solution: the 1-year mortality prediction model. We trained and compared six algorithms on the HD-478 cohort (478 patients, 19.9\% 1-year mortality) under stratified 10-fold cross-validation. We noticed that tree-based ensembles overfitted on this small cohort, while the \textbf{linear SVM with Yeo-Johnson preprocessing and Platt calibration} generalized best, achieving AUC\,=\,0.8272 and Brier\,=\,0.126. We then translated its margin score into three clinically actionable risk zones — Low, Moderate, and High — whose boundaries we derived from bootstrapped ROC/Youden analysis and grounded in KDIGO/DOPPS/TRIPOD guidelines.

\textbf{Chapter~6 — Backend \& Frontend Implementation} covers the software layer we built to bring these solutions to clinicians. We describe the Django REST API we structured around four independent applications (\texttt{users}, \texttt{patients}, \texttt{predictions}, \texttt{audit}), secured with JWT tokens and RBAC. We note that we chose a stateless prediction endpoint design to guarantee sub-100\,ms inference times. On the frontend, we present the React interface we designed for patient management, AI preprocessing review, prediction visualization (risk gauge and probability score), and performance monitoring.

\textbf{Chapter~7 — Deployment \& Evaluation} closes the report with the operational validation of our work. We describe the seven-service Docker Compose stack we assembled for one-command deployment, report on the system testing we conducted across all roles, and present the performance benchmarks we measured. We also honestly discuss the limitations we identified — single-center data, static model, no prospective validation — and propose the future directions we consider most promising, including federated learning and continuous model retraining.
...
\node[lbox,fill=softrose!15,draw=softrose,below=0.35cm of L2] (L3) {
  ...
  scikit-learn \textbullet\ SVM (C=0.05, linear) \textbullet\ KNNImputer \textbullet\ Yeo-Johnson\\
...
\chapteropening{5}{Machine Learning\\[0.2cm]Layer}{Model comparison, SVM selection, calibration, and risk zones.}{%
  ...
  \textbf{5.5}\hspace{8pt}Model Evaluation Metrics \dotfill \pageref{sec:metrics}\\[3pt]
  \textbf{5.6}\hspace{8pt}Model Comparison and Results \dotfill \pageref{sec:results}\\[3pt]
  \textbf{5.7}\hspace{8pt}Model Selection: Why SVM? \dotfill \pageref{sec:model_selection}\\[3pt]
  \textbf{5.8}\hspace{8pt}Probability Calibration and Risk Zones \dotfill \pageref{sec:calibration}\\[3pt]
  \textbf{5.9}\hspace{8pt}Prediction Workflow \dotfill \pageref{sec:pred_workflow}\\[3pt]
  \textbf{5.10}\hspace{8pt}Model Generalization and Statistical Validation \dotfill \pageref{sec:generalization}\\[3pt]
}
...
\textbf{Principle.} Logistic Regression models the probability of a binary outcome as a sigmoid function of a linear combination of input features. It learns one weight per feature, directly interpretable as a log-odds ratio: a positive weight means the feature increases mortality risk, a negative weight decreases it. L2 regularization (Ridge penalty) penalizes large weights, reducing overfitting.

\textbf{Role in the project.} We included Logistic Regression as the standard statistical baseline for binary clinical prediction. It serves as the reference against which all other models are compared: if a more complex algorithm does not outperform it, the added complexity is not justified. We used balanced class weighting to compensate for the 4:1 alive/deceased imbalance in the HD-478 cohort.

\config{LogisticRegression(C=0.1, penalty='l2', class\_weight='balanced', max\_iter=2000)}

\subsection{Support Vector Machine (Linear)}
\label{sec:svm}

\textbf{Principle.} The linear SVM \cite{cortes_vapnik_1995} finds the hyperplane that separates alive and deceased patients with the largest possible margin. When perfect separation is impossible — which is always the case in clinical data — it allows a controlled number of misclassifications through a regularization parameter $C$. Probabilities are obtained via \textbf{Platt scaling} \cite{platt1999}, which fits a sigmoid function on the SVM margin scores using 5-fold cross-validation, producing calibrated mortality probabilities rather than raw decision scores.

\textbf{Role in the project.} We selected the linear SVM as the deployed model because it generalizes well on small, imbalanced clinical cohorts. We set $C=0.05$ — a deliberately conservative value — to favour a wide, robust margin over perfect training accuracy on the 478-patient dataset. The resulting calibrated probability drives the three-zone risk stratification: Low ($\hat{p}<10\%$), Moderate ($10\%\leq\hat{p}<29\%$), High ($\hat{p}\geq29\%$). The model's feature weights are also directly exploitable to identify the top clinical drivers of each individual prediction.

\config{SVC(C=0.05, kernel='linear', class\_weight='balanced', probability=True, random\_state=42)}

\subsection{Random Forest}

\textbf{Principle.} Random Forest \cite{breiman_rf_2001} builds a large collection of decision trees, each trained on a random bootstrap sample of patients and a random subset of features at each split. The final prediction is the average vote across all trees. This diversity between trees reduces variance and makes the ensemble more robust than any individual tree.

\textbf{Role in the project.} We included Random Forest as the representative tree-based ensemble method, expected to capture non-linear interactions between clinical features — for example, the combined effect of low albumin and high Charlson index on mortality risk. We applied heavy regularization (max\_depth=3, min\_samples\_leaf=15) to limit overfitting on the small cohort. However, we observed that Random Forest (AUC\,=\,0.750) offered no advantage over linear models, confirming that non-linear boundaries are not beneficial at this sample size.

\config{RandomForestClassifier(n\_estimators=500, max\_depth=3, min\_samples\_leaf=15, class\_weight='balanced', oob\_score=True)}

\subsection{Linear Discriminant Analysis (LDA)}

\textbf{Principle.} LDA finds the linear projection of the feature space that best separates the two classes by maximizing the ratio of between-class variance to within-class variance. It assumes that both classes follow Gaussian distributions with equal covariance, and uses that structure to build a probabilistic classifier.

\textbf{Role in the project.} We included LDA as a probabilistic linear baseline complementary to the SVM. Its Gaussian assumptions are broadly compatible with continuous clinical variables (albumin, creatinine, Kt/V) after Yeo-Johnson normalization. We applied Ledoit-Wolf shrinkage (shrinkage=0.3) to stabilize the covariance estimate given the limited sample. LDA achieved the second-best F1-score (0.563), confirming that linear models are well-suited to this clinical problem, but its calibration was slightly inferior to the SVM (Brier 0.130 vs. 0.126).

\config{LinearDiscriminantAnalysis(solver='lsqr', shrinkage='auto')}

\subsection{LASSO (L1 Logistic Regression)}

\textbf{Principle.} LASSO applies L1 regularization to Logistic Regression. Unlike L2, the L1 penalty has the property of driving some feature weights exactly to zero, performing automatic feature selection: features with a zero weight are deemed non-contributive by the model. The stronger the regularization, the sparser the model.

\textbf{Role in the project.} We included LASSO to assess whether a sparse feature subset could perform comparably to the full 32-feature model, and to identify which features the regularizer eliminates. With strong regularization ($C=0.05$), LASSO ranked last among the six models (AUC\,=\,0.745), suggesting that the majority of the 32 engineered features carry genuine predictive signal and that aggressive sparsity is counterproductive on this cohort.

\config{LogisticRegression(C=0.05, penalty='l1', solver='saga', class\_weight='balanced', max\_iter=2000)}

\subsection{XGBoost}

\textbf{Principle.} XGBoost \cite{chen_xgboost_2016} builds an ensemble of shallow decision trees sequentially: each new tree is trained to correct the residual prediction errors of all previous trees. Regularization on the number of leaves and leaf weights controls overfitting. The sequential boosting strategy typically achieves state-of-the-art results on tabular data.

\textbf{Role in the project.} We included XGBoost as the representative gradient boosting method, which frequently dominates benchmark competitions on clinical tabular datasets. Despite aggressive regularization (max\_depth=1, n\_estimators=30) and explicit class imbalance correction (scale\_pos\_weight\,=\,$n_{\text{neg}}/n_{\text{pos}}$\,=\,4.03), XGBoost ranked 4th (AUC\,=\,0.754) and exhibited the largest train/test gap (+0.096) among all models — confirming that even shallow boosting overfits on the 478-patient cohort.

\config{XGBClassifier(n\_estimators=30, max\_depth=1, learning\_rate=0.2, subsample=0.7, colsample\_bytree=0.7, scale\_pos\_weight=4, eval\_metric='logloss', random\_state=42)}
...
\textbf{Principle.} KNN imputation fills a missing feature value by averaging that feature across the $k=3$ most similar patients (neighbors), where similarity is measured on the features that are present. Unlike mean or median imputation — which assigns the same global average to every patient regardless of their profile — KNN preserves the local structure of the data, producing estimates that reflect each patient's individual clinical context.

\textbf{Role in the project.} Several patients in the HD-478 cohort had missing values in up to 5 of the 32 features. We chose KNN ($k=3$) because clinical variables in this cohort are correlated: albumin, creatinine, and Charlson index tend to co-vary, so a patient's missing value is best estimated from neighbours who share a similar overall clinical profile. We set a tolerance of 5 missing features per patient; beyond that, the prediction is flagged as unreliable and surfaced to the clinician in the interface.
...
\textbf{Principle.} The Yeo-Johnson power transformation \cite{yeo_johnson_2000} applies a feature-specific exponent, estimated by maximum likelihood on the training set, that reduces skewness and brings each feature's distribution closer to Gaussian. Unlike Box-Cox, it handles both positive and negative values. Z-score standardization (zero mean, unit variance) is then applied automatically, placing all 32 features on a comparable scale before the SVM computes distances.

\textbf{Role in the project.} Clinical variables such as creatinine, ferritin, and proteinuria are strongly right-skewed: a small number of extreme values distort the SVM's maximum-margin boundary. We applied this transformation to all 32 features before training, freezing the fitted parameters on the training set. At inference time, the same parameters are reloaded and applied to each new patient's feature vector — ensuring that no information from future patients leaks into the preprocessing step.
...
The binary outcome variable \texttt{deces\_1an} is defined as 1 if the patient died within 365 days of dialysis initiation, and 0 otherwise. Out of 478 patients, 95 (19.9\%) are positive cases (deceased). This variable serves as the supervision signal for all six trained models.
...
\midrule
\textbf{Linear SVM} & \textbf{\texttt{C=0.05, kernel='linear', class\_weight='balanced'}} \\
...
\end{tabularx}
\smallskip\\
\footnotesize\textit{The SVM regularization parameter $C=0.05$ enforces strong regularization, suited to the high-dimensional feature space relative to the small cohort ($n=478$). Class imbalance is handled via \texttt{class\_weight='balanced'} for SVM, LR, LASSO and via \texttt{scale\_pos\_weight} for XGBoost.}
\end{table}
...
Before presenting the comparative results, we define the four evaluation metrics used throughout this chapter to assess and compare model performance.
...
\begin{itemize}
  \item \textbf{Offline model} (research phase): The SVM was trained and evaluated in a standalone Python environment on the HD-478 dataset using the tight pipeline. The 10-fold stratified cross-validation yielded \textbf{AUC\,=\,0.8272\,$\pm$\,0.078}. This is the value reported in all comparison tables and figures in Section~\ref{sec:results}.

  \item \textbf{Platform model} (deployed): After deploying the platform and integrating the HD-478 data through the LLM preprocessing pipeline, the model was \textbf{retrained via the platform's training endpoint} (\texttt{POST /api/predictions/train/}) on the preprocessed and validated database. This retrained model achieves \textbf{AUC\,=\,0.8272}, stored in \texttt{last\_mortalite.json} and displayed in the platform dashboard.
\end{itemize}

Both models converge to \textbf{AUC\,=\,0.8272}, confirming that the LLM preprocessing pipeline preserves — and does not degrade — model performance while delivering measurable improvements in data quality (fewer erroneous values, better-imputed missing fields, standardized units). The \textbf{platform model (AUC\,=\,0.8272) is therefore the reference model for clinical deployment}.

\subsection{PR-AUC and F1-score: Performance Under Class Imbalance}

Given the class imbalance noted above (19.9\% mortality), the \textbf{Precision-Recall AUC (PR-AUC)} complements the AUC-ROC by focusing exclusively on the model's ability to correctly identify the minority class. It measures the trade-off between:
\begin{align}
...
The SVM achieves \textbf{PR-AUC\,=\,0.484} and \textbf{F1\,=\,0.551}, the highest among all evaluated models. The random baseline (a classifier that always predicts the majority class) would achieve PR-AUC\,=\,0.199 (equal to the class prevalence), so the SVM's 0.484 represents a substantial gain. Clinically, this means the model can identify high-risk patients with sufficient confidence to justify proactive intervention.

\subsection{Brier Score: Probability Calibration Quality}

The \textbf{Brier Score} \cite{brier_1950} measures the average squared gap between each patient's predicted mortality probability and their actual outcome (0\,=\,alive, 1\,=\,deceased). The closer the predicted probability is to the true outcome for every patient, the lower — and better — the score. A score of 0 is perfect; a naive model predicting the cohort prevalence (0.199) for every patient would achieve BS\,=\,0.159. The SVM after Platt scaling achieves \textbf{BS\,=\,0.126}, which is below this baseline and below the 0.15 excellence threshold. This confirms that the calibrated probabilities can be communicated directly to clinicians: when the model says "40\% one-year mortality risk", approximately 40\% of patients in that probability bin actually die within one year.
...
\midrule
\rowcolor{medgreen!20}\textbf{\#1} & \textbf{Linear SVM} & \textbf{0.8272} & \textbf{0.078} & \textbf{0.8755} & \textbf{+0.048} & \textbf{0.126} & \textbf{0.551} & \textbf{\ding{51}} \\
...
\end{tabular}}
\smallskip\\
\footnotesize\textit{All values: 10-fold stratified CV on raw HD-478 data (tight pipeline). Gap = Train AUC $-$ Test AUC. Brier Score: lower is better (perfect = 0). The deployed platform model, retrained on the LLM-preprocessed database, also achieves \textbf{AUC\,=\,0.8272}, confirming that preprocessing quality preserves model performance (see Section~\ref{sec:metrics}).}
\end{table}


\begin{resultbox}{SVM Selected as Best Model}
In the 10-fold stratified CV evaluation on the HD-478 cohort, the linear SVM achieved the highest AUC-ROC (\textbf{0.8272\,$\pm$\,0.078}), lowest Brier Score (0.126), and competitive F1-score (0.551). The deployed platform model, retrained on the LLM-preprocessed database, also achieves \textbf{AUC\,=\,0.8272}, confirming that data quality improvements from the preprocessing pipeline preserve model performance. SVM was preferred for three clinical reasons: (1) linear boundary provides direct feature-weight interpretation via coefficients; (2) strong L2 regularization ($C=0.05$) prevents overfitting on the small cohort ($n=478$); (3) single-model structure facilitates regulatory compliance and audit traceability.
\end{resultbox}

\subsection{Visual Performance Analysis}

The ROC curve plots Sensitivity (True Positive Rate) against $1-$Specificity (False Positive Rate) for all decision thresholds; the dashed diagonal represents random chance (AUC\,=\,0.50). Figure~\ref{fig:roc_curves} shows that the three linear models --- \textbf{Linear SVM} (AUC\,=\,0.8272), \textbf{LDA} (0.8225), and \textbf{Logistic Regression} (0.8210) --- form a tight cluster well above all other models. \textbf{XGBoost} (0.7545), \textbf{Random Forest} (0.7496), and \textbf{LASSO} (0.7451) score significantly lower, confirming that linear decision boundaries generalize better on this 478-patient cohort with strong regularization. All models substantially outperform random chance.
...
The \textbf{Linear SVM} achieves the best score (0.8272\,$\pm$\,0.078), followed by \textbf{LDA} (0.8225\,$\pm$\,0.085) and \textbf{Logistic Regression} (0.8210\,$\pm$\,0.080). The three linear models form a tight cluster well above the tree-based methods: \textbf{XGBoost} (0.7545\,$\pm$\,0.088), \textbf{Random Forest} (0.7496\,$\pm$\,0.073), and \textbf{LASSO} (0.7451\,$\pm$\,0.098) score significantly lower, confirming that linear decision boundaries generalize better on this 478-patient cohort.
...
\FloatBarrier
Figure~\ref{fig:auc_gap_overfitting} shows the train-test AUC gap for each model. A gap above 0.10 (orange dashed line) signals significant overfitting; below 0.05 (green dashed line) is excellent. The \textbf{Linear SVM} records the lowest positive gap (+0.048, green zone), confirming excellent generalisation thanks to its strong L2 regularisation ($C=0.05$). \textbf{LDA} shows a similar gap (+0.047), confirming that both linear models generalize equally well. \textbf{Random Forest} shows a negative gap ($-0.013$), meaning the OOF test AUC slightly exceeds train AUC --- a sign of stable variance reduction from bagging. \textbf{Logistic Regression} (+0.059) and \textbf{LASSO} (+0.078) remain in the acceptable zone. \textbf{XGBoost} (+0.096) is the closest to the overfitting threshold, reflecting the tendency of boosting methods to memorise training patterns on small datasets. \textbf{All six models stay below 0.10}, confirming the absence of significant overfitting on the HD-478 cohort.

%--- Figure 5.5 : Train-test AUC gap (PNG) ---
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{screen/Train-test_AUC_gap_per_model.png}
\caption{Train-test AUC gap per model --- green\,$<$\,0.05 (excellent), orange\,$<$\,0.10 (acceptable). Linear SVM\,: lowest gap (0{,}048).}
...
%--- SVM ---
The \textbf{Linear SVM} (Figure~\ref{fig:confusion_svm}, threshold\,=\,0.245) achieves \textbf{65 true positives} (TP: deaths correctly flagged) and \textbf{307 true negatives} (TN: survivors correctly identified). It misses \textbf{29 deaths} (FN) and generates \textbf{77 false alarms} (FP). Sensitivity\,=\,69.1\% (65/94) and specificity\,=\,79.9\% (307/384). The threshold of 0.245 corresponds to the Youden-optimal operating point balancing sensitivity and specificity on the HD-478 cohort.
...
\caption{Confusion matrix --- Linear SVM (AUC\,=\,0.8272, threshold\,=\,0.245).}
\label{fig:confusion_svm}
\end{figure}

\FloatBarrier
%--- LDA ---
The \textbf{LDA} (Figure~\ref{fig:confusion_lda}, threshold\,=\,0.220) shows a different trade-off: \textbf{67 true positives} and \textbf{307 true negatives}, with \textbf{27 false negatives} and \textbf{77 false positives}. Sensitivity\,=\,71.3\% (67/94) and specificity\,=\,79.9\% (307/384). Compared to the SVM, LDA detects \textbf{2 more deaths} (67 vs.\ 65) and misses \textbf{2 fewer deaths} (27 vs.\ 29), resulting in slightly \textbf{higher sensitivity} (71.3\% vs.\ 69.1\%) at an equal specificity (79.9\%).
...
\FloatBarrier
%--- Logistic Regression ---
The \textbf{Logistic Regression} (Figure~\ref{fig:confusion_lr}, threshold\,=\,0.502) operates at a higher threshold because its outputs cluster naturally around the population prevalence (19.9\%). It achieves \textbf{68~TP}, \textbf{298~TN}, \textbf{26~FN}, \textbf{86~FP} --- sensitivity\,=\,72.3\%, specificity\,=\,77.6\%. Among the three models, \textbf{LR achieves the highest sensitivity} (72.3\%) and \textbf{lowest false negative count} (26 missed deaths), followed by LDA (71.3\%, FN=27) and SVM (69.1\%, FN=29). However, the SVM was selected for its \textbf{best AUC-ROC} (0.8272), \textbf{best probability calibration} (Brier\,=\,0.126), and \textbf{linear interpretability} --- properties that outweigh the marginal sensitivity difference of 3.2 percentage points in a clinical AI deployment context.
...
%=======================================================
Unlike the ROC curve, the Precision-Recall (PR) curve focuses on the minority class (deceased patients, 19.9\%) and is more informative under class imbalance. Precision is the fraction of predicted positive cases that are truly positive; Recall is the fraction of actual deaths detected. The baseline is a horizontal dashed line at the prevalence (0.199). Figure~\ref{fig:pr_curve} confirms that the \textbf{Linear SVM} achieves the best PR-AUC (0.484), maintaining the highest precision at all recall levels. \textbf{Logistic Regression} (0.478) and \textbf{LDA} (0.463) follow closely, while tree-based models drop faster as recall increases, confirming their lower quality under class imbalance.
...
The linear SVM achieved the highest AUC-ROC (0.8272\,$\pm$\,0.078) and lowest Brier Score (0.126) simultaneously among all six models. LDA came very close (AUC\,=\,0.8225) but shows a slightly higher Brier Score (0.130). XGBoost (0.7545) and Random Forest (0.7496) trail significantly, confirming that linear decision boundaries generalize better than tree-based methods on this 478-patient cohort with strong regularization.

\subsection{Suitability for Small Clinical Datasets}

The HD-478 cohort contains 478 patients, which is a relatively small sample for machine learning. On small, imbalanced datasets, tree-based ensembles tend to overfit, as evidenced by the train-test AUC gap analysis (Figure~\ref{fig:auc_gap_overfitting}). The SVM's strong regularization ($C = 0.05$) forces a large-margin hyperplane that generalizes well: its train-test AUC gap of 0.048 is among the lowest of all models, well below the overfitting threshold of 0.1.
...
\begin{enumerate}[noitemsep]
  \item Best AUC-ROC (0.8272) and Brier Score (0.126) among all 6 models; F1\,=\,0.551
  \item Lowest overfitting gap (0.048) — best generalization on this 478-patient cohort
...
The raw LinearSVC output is a signed margin score — a real number that can be large positive or negative — which does not directly represent a probability. \textbf{Platt scaling} \cite{platt1999} converts this margin score into a calibrated probability by fitting a sigmoid curve (a logistic function) on the margin values using 5-fold cross-validation, implemented through \texttt{CalibratedClassifierCV(method='sigmoid', cv=5)}. The result is a value $\hat{p} \in [0,1]$ that estimates the patient's observed mortality rate at the cohort level.

We also evaluated an isotonic regression calibrator \cite{zadrozny_isotonic_2002} (\texttt{iso\_calibrator.joblib}), trained on 10-fold out-of-fold probabilities. However, on the HD-478 cohort (478 patients), the isotonic calibrator produced a \textbf{degenerate plateau}: the vast majority of patients were mapped to a single value ($\hat{p}_{\text{iso}} = 0.189$), regardless of their individual SVM score. This complete loss of discrimination — only 2 distinct output levels over the entire cohort — makes the isotonic layer clinically unusable for individual risk scoring. The Platt-scaled probability $\hat{p}$ is therefore used directly for both the displayed risk score and zone classification.

Calibration quality is quantified by the \textbf{Brier Score} (\textbf{BS\,=\,0.126}), confirming that the predicted probabilities are well-aligned with observed mortality rates. A full discussion of this metric and its clinical interpretation is provided in Section~\ref{sec:metrics}.
...
\section{Prediction Workflow}\label{sec:pred_workflow}
...
\node[pipe] (p5) at (0,-6.0)
  {\textbf{5.}~SVM inference --- \texttt{SVC(C=0.05, linear)} $\longrightarrow$ $p_{\text{raw}}$};
...
The deployment-level workflow integrating this ML pipeline with the clinical interface, the REST API, and the database layer is detailed in Section~6.6.3 (Figure~6.50).
...
The train-test AUC gap of \textbf{0.048} (well below the 0.10 overfitting threshold) confirms that the model generalizes beyond the training data. The SVM's strong L2 regularization ($C=0.05$) forces a large-margin hyperplane that avoids memorizing individual patient records. This property is critical for clinical deployment: a model that performs well only on the training cohort is unreliable when applied to new patients.
...
A \textbf{nested cross-validation} (outer: 10 folds, inner: 5-fold hyperparameter search) returned \textbf{AUC\,=\,0.8241}, nearly identical to the standard 10-fold result (0.8272). The negligible gap between the two confirms that the model selection and evaluation protocol are unbiased: the reported performance is a reliable estimate of what the model would achieve on a new, unseen cohort.
...
\draw[fwd] (14.5,-3.2) -- node[msglbl]{check\_password() OK (PBKDF2-SHA256)} (7.5,-3.2);
...
The JWT payload encodes: \texttt{user\_id}, \texttt{id}, \texttt{email}, \texttt{nom}, \texttt{prenom}, \texttt{telephone}, \texttt{role} (string), \texttt{exp} (expiration Unix timestamp), and \texttt{iat} (issued-at timestamp) — added via \texttt{CustomTokenSerializer.get\_token()}. The server verifies the token signature using \textbf{HMAC-SHA256 (HS256)} with a 256-bit secret key stored in the environment variable \texttt{SECRET\_KEY}, which is never committed to version control.
...
\begin{itemize}
  \item \textbf{Platform logo and branding} at the top (\textit{AI NÉPHROCARE}), with a gradient typography
...
\begin{figure}[H]
\centering
\IfFileExists{carnet.png}{\includegraphics[width=0.40\textwidth]{carnet.png}}{\framebox[0.40\textwidth][c]{\small[Screenshot: Digital Notepad floating notepad]}}
...
\begin{figure}[htbp]
\centering
\IfFileExists{preproc_export_donnees.png}{\includegraphics[width=0.85\textwidth]{preproc_export_donnees.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: ``Data'' sheet --- LLM-modified cells highlighted in yellow]}}
...
\begin{figure}[htbp]
\centering
\IfFileExists{preproc_export_corrections.png}{\includegraphics[width=0.85\textwidth]{preproc_export_corrections.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: ``Corrections'' sheet --- summary table of all modifications]}}
...
\begin{figure}[htbp]
\centering
\IfFileExists{preproc_integration_choice.png}{\includegraphics[width=0.75\textwidth]{preproc_integration_choice.png}}{\framebox[0.75\textwidth][c]{\small[Screenshot: Version selector --- corrected vs original]}}
...
\begin{figure}[htbp]
\centering
\IfFileExists{screen/Devenir des patients.png}{\includegraphics[width=0.80\textwidth]{screen/Devenir des patients.png}}{\framebox[0.80\textwidth][c]{\small[Screenshot: Patient outcome --- alive/deceased distribution]}}
...
\draw[msgfwd] (14,-6.2)  -- node[msglbl,xshift=1cm]{SVC(C=0.05, linear)} (14,-6.2);
...
%============================================================
%  CHAPTER 7 — DEPLOYMENT & EVALUATION
%============================================================
\chapteropening{7}{Deployment\\[0.2cm]\& Evaluation}{Docker deployment, system testing, and results.}{%
...
\begin{figure}[htbp]
  ...
  \caption{Docker Compose stack — seven interconnected services of the AI NéphroCare platform}
...
\begin{table}[htbp]
\centering
\caption{Prediction endpoint latency (measured over 25 requests, 5 distinct patients)}
\label{tab:latency}
\begin{tabular}{lcc}
\toprule
\textbf{Operation} & \textbf{Mean (ms)} & \textbf{P95 (ms)} \\
\midrule
Feature extraction & 10 & 15 \\
KNN imputation & 7 & 11 \\
Yeo-Johnson + SVC & 20 & 26 \\
Risk zone assignment & 3 & 3 \\
\midrule
\textbf{Total prediction} & \textbf{40} & \textbf{55} \\
\bottomrule
\end{tabular}
\end{table}

The mean end-to-end prediction latency of \textbf{40ms} (P95\,=\,55ms) is well within the $<$200ms threshold considered acceptable for interactive clinical applications.

\subsection{Data Processing Time}

\begin{itemize}
  \item Excel import (478 rows): $\sim$1--2 seconds (file parsing + PostgreSQL bulk insert)
  \item LLM preprocessing (478 rows, async): $\sim$1--2 minutes (qwen2.5:14b, GPU-accelerated)
  \item Model training on 478 patients: $\sim$2 seconds (10-fold stratified CV + final fit)
\end{itemize}

The LLM preprocessing time of 1--2 minutes for 478 rows reflects the sequential processing approach used to preserve the quality of Qwen2.5:14B analysis --- each row is processed individually with full clinical context. For routine imports of smaller datasets (10--50 new patients), the processing time is proportionally reduced to under 10 seconds. The asynchronous Celery architecture ensures that this processing time does not block the user interface: clinicians can continue using other platform features while the LLM analysis runs in the background. The SVM training time of $\sim$2 seconds reflects the efficiency of linear kernel SVMs on small clinical datasets: the $\mathcal{O}(n^2)$ complexity of the QP solver is negligible for $n=478$, making retraining practical whenever new validated patients are integrated.
...
\subsection{Decision Support Value}\label{sec:decision_support}
...
We identified several limitations in the current version of the platform that should be addressed in future iterations.

\begin{itemize}
  \item \textbf{Cohort size, selection bias, and single-center data.} The 478-patient HD-478 cohort is adequate for initial validation but limits subgroup stratification (e.g., per-etiology analysis with fewer than 20 patients per category). Selection bias may also arise from the exclusion of patients lost to follow-up, who may differ systematically from those with complete records. The model has not been externally validated on data from other centers, and its generalizability to different patient populations and clinical practices remains to be established.
  \item \textbf{Population specificity.} The model was trained on a predominantly Moroccan hemodialysis population with a specific etiological distribution (diabetic nephropathy, hypertensive nephrosclerosis dominant). Feature weights and risk thresholds may not transfer directly to European, North American, or sub-Saharan African HD centers. Recalibration on the local population is required before deployment in a different institutional context.
  \item \textbf{Baseline-only features.} The SVM is trained exclusively on admission-time features and does not incorporate longitudinal evolution (e.g., 6-month Kt/V trajectory or albumin trends), which are known prognostic signals in the HD population.
  \item \textbf{Probabilistic uncertainty.} The model outputs a population-calibrated probability, not an individual certainty. Communicating this distinction to clinicians requires dedicated training and appropriate interface design to avoid overconfidence in the predictions.
  \item \textbf{GPU dependency.} LLM preprocessing relies on Qwen2.5:14B with GPU acceleration via Ollama. While this delivers inference times of 1--2 minutes for 478 rows, CPU-only deployments would be significantly slower, which may be a constraint in resource-limited hospital environments.
\end{itemize}

\section*{Perspectives}

We identify the following directions as the most promising extensions to the current platform.

\begin{itemize}
  \item \textbf{External multi-center validation.} The most urgent priority is to validate the model on an independent cohort from a different center or country, following TRIPOD reporting standards, to assess its generalizability and calibration in diverse clinical settings.
  \item \textbf{Real-time hospital integration via HL7 FHIR.} Integration with the CHU Hassan II Hospital Information System through HL7 FHIR RESTful APIs would eliminate manual Excel imports and enable continuous, real-time patient risk monitoring as new clinical data is recorded.
  \item \textbf{Longitudinal and deep learning models.} For larger future cohorts ($>2{,}000$ patients), transformer-based architectures on longitudinal lab sequences (e.g., lab-BERT) or graph neural networks encoding comorbidity co-occurrence patterns could substantially improve predictive performance over the current baseline-only SVM.
  \item \textbf{Cohort-portable retraining.} A natural extension is to allow any institution to import its own hemodialysis cohort and automatically retrain the full ML pipeline on that data, producing cohort-specific risk models without any code modification. Because our pipeline operates exclusively on the data stored in the platform database, this architecture is already largely in place.
  \item \textbf{Federated learning.} In the longer term, federated learning across multiple nephrology centers would allow a shared, privacy-preserving model to be trained on combined multi-center data without any patient record leaving its originating institution, addressing both the cohort size limitation and regulatory constraints on data sharing.
\end{itemize}

\section*{Conclusion}

Through this project, we designed, developed, and validated \textbf{AI NéphroCare}, a full-stack intelligent clinical decision support platform for 1-year mortality prediction in hemodialysis patients at CHU Hassan II of Fès, Morocco.

We observed at the outset that the nephrology unit held years of rich clinical data — 478 patient records across 91 variables — that were entirely siloed in Excel files, riddled with quality issues, and never exploited for decision support. We addressed this through four interconnected solutions: (1) \textit{data centralization} — we built a structured 160+-field database that unifies all patient records for the first time; (2) \textit{intelligent preprocessing} — we deployed an LLM+RAG pipeline (Qwen2.5:14B + ChromaDB) that automatically detects and corrects data quality issues, substantially reducing manual data cleaning effort (see Section~\ref{sec:decision_support} for quantified results); (3) \textit{predictive modeling} — we systematically compared six machine learning algorithms and noticed that a well-regularized linear SVM with Yeo-Johnson preprocessing and Platt calibration outperformed more complex ensemble methods on this small-scale, imbalanced cohort, achieving AUC\,=\,0.8272 and Brier\,=\,0.126; and (4) \textit{clinical usability} — we embedded the model in a React-based clinical interface with a visual risk gauge, top-5 contributing factors, audit trail, and role-based access control.

The three evidence-based risk zones defined in Chapter~5 (Low / Moderate / High, with thresholds T1\,=\,10\% and T2\,=\,29\% grounded in TRIPOD/DOPPS/KDIGO guidelines) provide clinicians with actionable guidance, with a relative risk ratio of $\times$12.9 between extreme zones. From a software engineering standpoint, the integration of all pipeline components into a single seven-service Docker Compose stack demonstrates the feasibility of deploying sophisticated clinical AI tools in resource-constrained hospital environments without specialized MLOps infrastructure.
...
\fancyhead[LE,LO]{\small Bibliography}
\fancyhead[RE,RO]{}
...
\newblock \textit{Django REST Framework --- Documentation}.
\newblock \url{https://www.django-rest-framework.org/}. Accessed June 2026.

\bibitem{react_docs}
Meta Platforms, Inc. (2024).
\newblock \textit{React --- The library for web and native user interfaces}.
\newblock \url{https://react.dev/}. Accessed June 2026.

\bibitem{sklearn_docs}
Pedregosa F, et al. (2011).
\newblock \textit{scikit-learn: Machine Learning in Python --- Documentation}.
\newblock \url{https://scikit-learn.org/stable/}. Accessed June 2026.

\bibitem{ollama_docs}
Ollama (2024).
\newblock \textit{Ollama --- Run large language models locally}.
\newblock \url{https://ollama.ai/}. Accessed June 2026.

\bibitem{chromadb_docs}
Chroma (2024).
\newblock \textit{ChromaDB --- The AI-native open-source embedding database}.
\newblock \url{https://docs.trychroma.com/}. Accessed June 2026.

\bibitem{kdigo_web}
Kidney Disease: Improving Global Outcomes (KDIGO) (2024).
\newblock \textit{KDIGO Clinical Practice Guidelines}.
\newblock \url{https://kdigo.org/guidelines/}. Accessed June 2026.

\bibitem{mui_docs}
MUI (2024).
\newblock \textit{Material UI --- React component library}.
\newblock \url{https://mui.com/}. Accessed June 2026.

\bibitem{chartjs_docs}
Chart.js Contributors (2024).
\newblock \textit{Chart.js --- Simple yet flexible JavaScript charting}.
\newblock \url{https://www.chartjs.org/}. Accessed June 2026.

\bibitem{postgresql_docs}
The PostgreSQL Global Development Group (2024).
\newblock \textit{PostgreSQL 16 Documentation}.
\newblock \url{https://www.postgresql.org/docs/16/}. Accessed June 2026.

\bibitem{docker_docs}
Docker, Inc. (2024).
\newblock \textit{Docker Documentation}.
\newblock \url{https://docs.docker.com/}. Accessed June 2026.
