...
\begin{itemize}
  ...
  \item \textbf{Vascular access type}: Arteriovenous fistula (AVF) use is a strong protective factor; tunneled catheters are associated with approximately 2$\times$ higher infection-related mortality risk \cite{pisoni2002vascular}.
...
\small\begin{tabularx}{\textwidth}{p{4.5cm}p{2.2cm}X}
\toprule
...
\texttt{demographie\_couverture\allowbreak\_sociale} & TEXT & Social coverage (RAMED, AMO, etc.) \\
...
\texttt{dialyse\_seances\allowbreak\_par\_semaine} & TEXT & Sessions per week \\
...
\toprule
\textbf{Domain} & \textbf{Features} & \textbf{Rationale / Reference} \\
\midrule
Biological markers & \texttt{albumine\_basale}, \texttt{calcium\_basale}, \texttt{ferritine\_basale}, \texttt{pth\_basale} & Low albumin ($<$35 g/L) and abnormal mineral metabolism are independent mortality predictors \cite{albumin_dialysis_2018, kdigo_mineral_2017, kdigo_anemia_2012} \\
\addlinespace
Dialysis modality \& quality & \texttt{hemodialyse}, \texttt{seances\_par\_semaine}, \texttt{fistule\_arterioveineuse\_creee}, \texttt{admission\_cathetere\_tunnellise}, \texttt{du\_residuelle} & Dialysis frequency, vascular access type, and residual renal function are key KDIGO and DOPPS mortality indicators \cite{kdigo2015, pisoni2002vascular, shemin2001residual, dopps2022} \\
\addlinespace
Cardiovascular comorbidities & \texttt{hypertension}, \texttt{cardiopathie}, \texttt{evenement\_cardiovasculaire}, \texttt{diabete} & Cardiovascular disease is the leading cause of death in HD patients; diabetes and HTA amplify risk \cite{registre_rein_2022, beddhu2000charlson, dopps2022} \\
\addlinespace
Clinical symptoms & \texttt{dyspnee}, \texttt{oedemes\_surcharge}, \texttt{douleur\_abdominale}, \texttt{crise\_convulsive} & Symptom burden (dyspnea, edema) is a marker of decompensation and predicts short-term mortality in dialysis cohorts \cite{registre_rein_2022} \\
\addlinespace
Disease context & \texttt{etiologie\_mrc}, \texttt{maladie\_renale\_hereditaire}, \texttt{nombre\_hospitalisations}, \texttt{annee\_inclusion} & CKD etiology, hereditary disease, and hospitalisation count are prognostic covariates validated in both registry and ML-based studies \cite{kdigo2015, sheng2020, maoujoud2017morocco} \\
\addlinespace
Care \& access & \texttt{couverture\_medicale}, \texttt{liste\_attente\_transplantation}, \texttt{information\_transplantation\_donnee} & Socioeconomic determinants and transplant trajectory are locally significant predictors specific to the Moroccan context \cite{maoujoud2017morocco, usrds2022} \\
\bottomrule
\end{tabular}
\end{table}

These features are extracted from Patient ORM objects by the function \texttt{extract\_features\_for\_patient()} in \texttt{feature\_mapping.py}, using a \textbf{multi-strategy extraction}: direct numeric fields, binary encoded fields, keyword-detected fields (e.g., dyspnea detected from symptom free-text), and ordinal categorical encoding.
...
This dual approach --- statistical and clinical --- reflects recognised practice in medical modelling, where variable selection cannot rely exclusively on algorithmic criteria, at the risk of excluding clinically relevant markers whose isolated effect is weak but whose presence is necessary for model interpretability by the clinician \cite{beddhu2000charlson, albumin_dialysis_2018, kdigo2015}.

Table~\ref{tab:var_evidence} details the clinical justification and key supporting reference for each of the 24 variables, together with their SHAP validation status.

\begin{longtable}{@{}p{4.2cm}p{1.5cm}p{5.0cm}p{2.8cm}@{}}
\caption{Per-variable clinical justification and supporting literature for the 24 selected features}
\label{tab:var_evidence}\\
\toprule
\textbf{Variable} & \textbf{Status} & \textbf{Clinical evidence} & \textbf{Reference} \\
\midrule
\endfirsthead
\multicolumn{4}{c}{\footnotesize\textit{(continued from previous page)}}\\
\toprule
\textbf{Variable} & \textbf{Status} & \textbf{Clinical evidence} & \textbf{Reference} \\
\midrule
\endhead
...
%--- SHAP-confirmed ---
\multicolumn{4}{l}{\textit{--- SHAP-confirmed (top-24 out of 77) ---}}\\[3pt]

\texttt{fistule\_arterioveineuse\allowbreak\_creee} & SHAP \#1 &
  AVF vs.\ tunnelled catheter: $-$26\% mortality risk in the DOPPS cohort; vascular access type is ranked among the top-15 predictors of 1-year HD mortality in the Sheng 2020 ML model &
  \cite{pisoni2002vascular, sheng2020} \\[4pt]

\texttt{couverture\_medicale} & SHAP \#2 &
  Systematic review of worldwide ESRD access: lack of treatment coverage directly increases mortality; in LMIC settings health coverage determines dialysis continuity and survival &
  \cite{liyanage2015, usrds2022} \\[4pt]

\texttt{calcium\_basale} & SHAP \#3 &
  Both hypercalcaemia and hypocalcaemia independently predict 1-year mortality across DOPPS sites; calcium is included among the 42 candidate features in the Sheng 2020 1-year HD mortality ML model &
  \cite{tentori2008, sheng2020} \\[4pt]

\texttt{albumine\_basale} & SHAP \#4 &
  Each 1~g/dL decrease in serum albumin increases mortality risk; albumin is a confirmed predictor in both the Kalantar-Zadeh cohort study and the Sheng 2020 XGBoost 1-year HD mortality model &
  \cite{albumin_dialysis_2018, sheng2020} \\[4pt]

\texttt{evenement\_cardiovasculaire} & SHAP top-24 &
  Ischemic heart disease and cerebrovascular disease are both top-15 predictors in the Sheng 2020 XGBoost 1-year HD mortality model; cardiovascular disease accounts for $>$40\% of HD deaths &
  \cite{foley1998cv, sheng2020} \\[4pt]

\texttt{admission\_cathetere\allowbreak\_tunnellise} & SHAP top-24 &
  Tunnelled catheter is associated with 2--3$\times$ higher infection risk vs.\ AVF; vascular access type (including catheter) is a top-15 predictor in the Sheng 2020 1-year HD mortality model &
  \cite{pisoni2002vascular, sheng2020} \\[4pt]

\texttt{du\_residuelle} & SHAP top-24 &
  Each 1~mL/min increase in residual GFR is associated with a 5--10\% reduction in mortality risk; residual function provides clearance that HD sessions cannot fully replace &
  \cite{shemin2001residual} \\[4pt]

\texttt{nombre\_hospitalisations} & SHAP top-24 &
  Prior hospitalisations are an independent predictor of early mortality among incident HD patients in the DOPPS international cohort; count captures total clinical burden beyond individual comorbidities &
  \cite{bradbury2007dopps, beddhu2000charlson} \\[4pt]

\texttt{pth\_basale} & SHAP top-24 &
  PTH $>$600~pg/mL associated with elevated all-cause mortality in the DOPPS cohort; parathyroid hormone is included among the 42 candidate features in the Sheng 2020 1-year HD mortality model &
  \cite{tentori2008, sheng2020} \\[4pt]

\texttt{ferritine\_basale} & SHAP top-24 &
  J-shaped relationship between ferritin and HD mortality confirmed in JASN cohort; ferritin is included among the 42 candidate features in the Sheng 2020 1-year HD mortality XGBoost model &
  \cite{kalantar2005iron, sheng2020} \\[4pt]

\texttt{seances\_par\_semaine} & SHAP top-24 &
  Longer treatment time and adequate session frequency are independently associated with reduced mortality in the DOPPS international cohort; each session below 3/week increases mortality risk &
  \cite{saran2006dopps} \\[4pt]

\texttt{etiologie\_mrc} & SHAP top-24 &
  CKD aetiology (diabetic nephropathy top-15 in Sheng 2020; also glomerulonephritis, hypertensive, hereditary) is an explicit feature set in the Sheng 2020 1-year HD mortality model &
  \cite{sheng2020, usrds2022} \\[4pt]

\texttt{annee\_inclusion} & SHAP top-24 &
  USRDS data show year-on-year improvement in HD 1-year survival; inclusion year captures era-specific protocol effects on outcome across the 2020--2024 cohort &
  \cite{usrds2022} \\[4pt]

%--- Maintained for clinical value ---
\multicolumn{4}{l}{\textit{--- Literature-justified variables (maintained for clinical phenotyping) ---}}\\[3pt]

\texttt{hypertension} & Lit. &
  Systolic and diastolic blood pressure are included in the Sheng 2020 1-year HD mortality model; HTA is the primary cardiovascular mortality driver and a Charlson index component &
  \cite{foley1998cv, beddhu2000charlson, sheng2020} \\[4pt]

\texttt{cardiopathie} & Lit. &
  Congestive heart failure and ischemic heart disease are both top-15 predictors in the Sheng 2020 XGBoost 1-year HD mortality model; cardiac disease accounts for $>$40\% of HD deaths &
  \cite{foley1998cv, sheng2020} \\[4pt]

\texttt{diabete} & Lit. &
  Diabetic nephropathy is a top-15 predictor in the Sheng 2020 1-year HD mortality model; it is also the leading cause of ESRD worldwide with amplified cardiovascular and infectious risk &
  \cite{sheng2020, usrds2022} \\[4pt]

\texttt{dyspnee} & Lit. &
  Dyspnoea is the primary clinical sign of fluid overload and cardiac decompensation; overhydration directly predicts cardiovascular mortality in HD &
  \cite{wizemann2009, foley1998cv} \\[4pt]

\texttt{oedemes\_surcharge} & Lit. &
  Chronic overhydration increases left ventricular mass and all-cause mortality; $>$2.5~L fluid excess associated with 2.1$\times$ relative risk of death &
  \cite{wizemann2009} \\[4pt]

\texttt{douleur\_abdominale} & Lit. &
  GI complications (peptic ulcers, liver disease, mesenteric ischaemia) are explicit components of the Charlson Comorbidity Index, whose score independently predicts clinical outcomes and mortality in dialysis patients &
  \cite{beddhu2000charlson} \\[4pt]

\texttt{crise\_convulsive} & Lit. &
  Seizures in HD signal uremic encephalopathy, severe electrolyte disorders or stroke; neurological and cerebrovascular events are Charlson index components and independent mortality predictors &
  \cite{beddhu2000charlson, foley1998cv} \\[4pt]

\texttt{maladie\_renale\_hereditaire} & Lit. &
  Polycystic kidney disease is an explicit etiology feature in the Sheng 2020 1-year HD mortality model; ERA-EDTA registry confirms ADPKD patients have better survival than diabetic ESRD patients &
  \cite{spithoven2014, sheng2020} \\[4pt]

\texttt{hemodialyse} & Lit. &
  Propensity-matched JASN study: HD and PD patients have significantly different 1-year mortality profiles; modality flag discriminates patients who switched to PD or other modalities during follow-up &
  \cite{weinhandl2010} \\[4pt]

\texttt{liste\_attente\_transplantation} & Lit. &
  Waitlisted HD patients have $\sim$50\% lower mortality than non-listed patients (USRDS); listing status reflects both clinical eligibility and care quality &
  \cite{usrds2022} \\[4pt]

\texttt{information\_transplantation\allowbreak\_donnee} & Lit. &
  Transplant education is associated with higher waitlisting rates; USRDS data show waitlisted patients have $\sim$50\% lower mortality than non-listed HD patients &
  \cite{usrds2022} \\[4pt]
...
\texttt{age\_x\_charlson} &
  ...
  \cite{beddhu2000charlson, khan2008score} \\[4pt]
...
\texttt{charlson\_x\_hosp} &
  ...
  \cite{sheng2020, beddhu2000charlson} \\[4pt]
...
Given the limited dataset size ($n=478$), an exhaustive GridSearchCV across all algorithms would risk overfitting the evaluation metric. Instead, each model's hyperparameters were selected through an iterative manual search guided by published benchmarks on similar clinical cohorts, then validated with the same 10-fold stratified CV used for model comparison (Section~\ref{sec:model_comparison}). No separate validation fold was held out for tuning — all hyperparameter choices are therefore conservative defaults rather than data-fitted optima, which strengthens the generalizability claim.

Table~\ref{tab:hyperparams} summarizes the final configuration retained for each of the nine algorithms.
...
\midrule
Logistic Regression & \texttt{C=0.5, solver='saga', class\_weight='balanced', max\_iter=500} \\
\textbf{SVM (Linear)} & \textbf{\texttt{C=0.01, kernel='linear', class\_weight='balanced', probability=True}} \\
Random Forest & \texttt{n\_estimators=200, class\_weight='balanced\_subsample', max\_features='sqrt'} \\
Extra Trees & \texttt{n\_estimators=200, class\_weight='balanced\_subsample'} \\
Gradient Boosting & \texttt{n\_estimators=150, learning\_rate=0.05, max\_depth=4, subsample=0.9} \\
AdaBoost & \texttt{n\_estimators=150, learning\_rate=0.1, base\_estimator=DT(max\_depth=2)} \\
XGBoost & \texttt{scale\_pos\_weight=4.03, n\_estimators=150, subsample=0.8, colsample\_bytree=0.8} \\
LightGBM & \texttt{n\_estimators=200, num\_leaves=31, learning\_rate=0.05, class\_weight='balanced'} \\
CatBoost & \texttt{iterations=200, depth=4, learning\_rate=0.05, l2\_leaf\_reg=3} \\
\bottomrule
\end{tabularx}
\smallskip\\
\footnotesize\textit{The SVM regularization parameter C=0.01 enforces strong regularization, well-suited to the high-dimensional feature space relative to the small cohort size. All ensemble models use \texttt{n\_estimators$\geq$150} to stabilize variance estimates. Class imbalance is handled via \texttt{class\_weight='balanced'} for SVM/LR/RF/ET and via \texttt{scale\_pos\_weight} for XGBoost.}
\end{table}
...
Nine models were trained and evaluated on the HD-478 cohort using 10-fold stratified cross-validation. Table~\ref{tab:model_comparison} reports the four primary metrics for each model.
...
Three additional models tested in the initial comparative study --- \textbf{LDA} (AUC\,=\,0.8225), \textbf{LR Ridge} (0.821), and \textbf{LASSO} (0.745) --- appear in the visual comparison figures (Figures~\ref{fig:roc_curves}--\ref{fig:boxplot_folds}) but are not included in this summary table, which focuses on the nine primary algorithms evaluated.}
\end{table}
...
\begin{enumerate}
  ...
  \item For every subsequent request the client attaches the header \texttt{Authorization: Bearer <token>}.
...
\textit{Actifs}, \textit{Inactifs}, \textit{Super Admins}, \textit{Heads of Dept.}, \textit{Professors}, \textit{Residents}, \textit{Rôles}.
...
When a Resident or Professor submits a preprocessed file for approval, the Head of Department receives a \textbf{notification badge} in the sidebar. Clicking \textbf{"Aperçu"} opens a preview modal showing file name, submitter, row/column counts, quality score, and the first 15 rows. From the modal, the Head of Department can \textbf{validate and integrate} (data inserted into the patient database) or close without action. A reject action is available via the detail endpoint with a mandatory reason.
...
\begin{figure}[H]
\centering
\IfFileExists{apercu_validation.png}{\includegraphics[width=0.85\textwidth]{apercu_validation.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Head of Department validation preview modal]}}
...
\begin{figure}[H]
\centering
\IfFileExists{dashboard_resident.png}{\includegraphics[width=0.85\textwidth]{dashboard_resident.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Professor/Resident dashboard --- profile + activity feed]}}
...
\begin{figure}[H]
\centering
\IfFileExists{screen/AI Model analysis dashb.png}{\includegraphics[width=0.85\textwidth]{screen/AI Model analysis dashb.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: AI Model analysis dashboard]}}
...
\begin{figure}[H]
\centering
\IfFileExists{screen/simulation.png}{\includegraphics[width=0.85\textwidth]{screen/simulation.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Simulation interface --- modified parameters and resulting risk prediction]}}
...
\begin{figure}[H]
\centering
\IfFileExists{screen/AI prediction result interface  risk zone gauge.png}{\includegraphics[width=0.85\textwidth]{screen/AI prediction result interface  risk zone gauge.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Prediction tab --- risk gauge, probability, top-5 factors, recommendation]}}
...
\begin{figure}[H]
\centering
\IfFileExists{screen/profile_page.png}{\includegraphics[width=0.85\textwidth]{profile_page.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Profile page --- user info, password change, language selector]}}
...
\begin{figure}[H]
\centering
\IfFileExists{screen/Cohort risk zone distribution.png}{\includegraphics[width=0.75\textwidth]{screen/Cohort risk zone distribution.png}}{\framebox[0.75\textwidth][c]{\small[Screenshot: AI risk zone distribution chart]}}
...
\begin{figure}[H]
\centering
\IfFileExists{screen/Devenir des patients.png}{\includegraphics[width=0.80\textwidth]{screen/Devenir des patients.png}}{\framebox[0.80\textwidth][c]{\small[Screenshot: Patient outcome --- vivant/décès distribution]}}
\caption{Patient outcome distribution (vivant / décès) across the hemodialysis cohort}
...
\begin{figure}[H]
\centering
\IfFileExists{screen/Kaplan-Meier.png}{\includegraphics[width=0.82\textwidth]{screen/Kaplan-Meier.png}}{\framebox[0.82\textwidth][c]{\small[Screenshot: Kaplan-Meier survival curves by risk zone]}}
...
\begin{figure}[H]
\centering
\IfFileExists{screen/Serum albumin distribution by risk zone.png}{\includegraphics[width=0.85\textwidth]{screen/Serum albumin distribution by risk zone.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Box plots of biological indicators by risk zone]}}
...
\node[actorbox] (DB) at (18.5,0)  {Redis / DB};
...
\draw[msgfwd] (3,-1.6)  -- node[msglbl]{2. POST /patients/preprocess/analyze/ (multipart)} (6.5,-1.6);
...
\draw[msgfwd] (9.5,-7.5) -- node[msglbl]{10. Save session \{status, report, corrections\}} (18.5,-7.5);
\draw[msgret] (18.5,-8.2) -- node[msglbl]{11. OK (Redis cache)} (9.5,-8.2);
\draw[msgfwd] (9.5,-8.9) -- node[msglbl]{12. Update session status = «completed» (Redis)} (18.5,-8.9);
...
The flow begins when the clinician clicks the \textit{Prédire} button (Step~1), which sends \texttt{GET /predictions/patient/\{patient\_id\}/mortalite/} to the backend (Step~2). The API retrieves the full patient record from PostgreSQL (Steps~3--4) and calls \texttt{extract\_features()} to build the clinical feature vector (Step~5). The vector is then passed to the SVM pipeline (Step~6), which applies the three preprocessing stages internally: KNN imputation ($k=3$) for missing values, Yeo-Johnson power transformation for normalization, and finally the linear SVC decision function. The raw probability $p_\text{raw}$ is returned (Step~7) and calibrated to a final probability $\hat{p}$ via Platt scaling (CalibratedClassifierCV, Step~8).

The backend then derives the risk score as $\text{score} = \hat{p} \times 100$ and assigns a categorical risk level using fixed clinical thresholds: \textbf{Faible} ($\hat{p} < 10\%$), \textbf{Modéré} ($10\% \leq \hat{p} < 29\%$), or \textbf{Élevé} ($\hat{p} \geq 29\%$). A clinical recommendation text and the list of most influential factors are assembled before the action is logged in \texttt{AuditLog} (Step~11). The structured JSON response (Step~13) contains \texttt{risk\_level}, \texttt{score}, \texttt{probability}, \texttt{factors}, \texttt{recommendation}, and \texttt{features\_used}, which the frontend uses to render the risk gauge and results panel (Step~14).
...
\draw[msgfwd] (3.5,-1.6) -- node[msglbl]{2. GET /predictions/patient/\{patient\_id\}/mortalite/} (7,-1.6);
...
\node[draw,fill=green!10,font=\tiny,align=left,inner sep=3pt] at (11.5,-10.3)
  {Faible: $\hat{p} < 10\%$\\Modéré: $10\% \leq \hat{p} < 29\%$\\Élevé: $\hat{p} \geq 29\%$};
...
The three-zone stratification — Faible ($\hat{p}<10\%$), Modérée ($10\%\leq\hat{p}<29\%$), Élevée ($\hat{p}\geq29\%$) — with thresholds derived by ROC/Youden bootstrap analysis, was confirmed clinically meaningful for daily triage and resource allocation.
...
\newblock PMID: 15928095. DOI: 10.1093/ndt/gfh941.
...
\newblock PMID: 18653268. DOI: 10.1053/j.ajkd.2008.05.015.
...
\newblock PMID: 16609682. DOI: 10.1038/sj.ki.5000186.

\bibitem{grantham2008adpkd}
Grantham JJ (2008).
\newblock Autosomal dominant polycystic kidney disease.
\newblock \textit{New England Journal of Medicine}, 359(14):1477--1485.
\newblock PMID: 18832244. DOI: 10.1056/NEJMcp0804458.

\bibitem{spithoven2014}
Spithoven EM, Kramer A, Meijer E, et al. (2014).
\newblock Renal replacement therapy for autosomal dominant polycystic kidney
  disease (ADPKD) in Europe: prevalence and survival --- an analysis of data
  from the ERA-EDTA Registry.
\newblock \textit{Nephrology Dialysis Transplantation}, 29(Suppl~4):iv15--iv25.
\newblock PMID: 25165187. DOI: 10.1093/ndt/gfu017.
...
\newblock \textit{Journal of the American Society of Nephrology},
  16(10):3070--3080.
\newblock PMID: 16093449. DOI: 10.1681/ASN.2005040423.
...
\bibitem{bradbury2007dopps}
Bradbury BD, Fissell RB, Albert JM, et al. (2007).
\newblock Predictors of early mortality among incident US hemodialysis patients
  in the Dialysis Outcomes and Practice Patterns Study (DOPPS).
\newblock \textit{Clinical Journal of the American Society of Nephrology},
  2(1):89--99.
\newblock PMID: 17699392. DOI: 10.2215/CJN.01170906.
