...
Nine machine learning algorithms were systematically evaluated using stratified 10-fold cross-validation. A \textbf{linear SVM} emerged as the best-performing model (AUC-ROC = 0.8262, PR-AUC = 0.4887, F1 = 0.5667, Brier Score = 0.1265), combining strong discriminative ability with clinical interpretability. Raw SVM probabilities are calibrated via Platt scaling (\texttt{SVC(probability=True)}, internal 5-fold sigmoid) and mapped to three evidence-based clinical risk zones, with thresholds derived by ROC curve analysis (T1 = 10\%, Sensitivity $\geq$ 90\%) and bootstrapped Youden index (T2 = 29\%, $n=1000$ iterations, median = 28.9\%): \textbf{Low Risk} ($\hat{p} < 10\%$, observed mortality 3.8\%), \textbf{Moderate Risk} ($10\% \leq \hat{p} < 29\%$, 15.6\%), and \textbf{High Risk} ($\hat{p} \geq 29\%$, 46.5\%).
...
\begin{enumerate}
  ...
  \item \textbf{Platt-scaled probability} (\texttt{SVC(probability=True)}, internal 5-fold sigmoid): Maps the SVM margin score to a calibrated probability $\hat{p}$
...
Probabilities are obtained using \textbf{Platt scaling} \cite{platt1999}: \texttt{SVC(probability=True)} fits a logistic regression $P(y=1 \mid f) = \sigma(Af + B)$ on the signed margin distances $f_i = \mathbf{w}^T\mathbf{x}_i + b$ via internal 5-fold cross-validation.
...
\config{solver='svd', shrinkage=None, \texttt{class\_prior=None}}
...
\config{C=0.1, penalty='l1', solver='liblinear', class\_weight='balanced'}
...
LDA & \texttt{solver='svd', shrinkage=None} \\
Logistic Regression & \texttt{C=0.5, solver='saga', penalty='l2', class\_weight='balanced', max\_iter=500} \\
...
LASSO & \texttt{C=0.1, penalty='l1', solver='liblinear', class\_weight='balanced'} \\
\bottomrule
\end{tabularx}
\smallskip\\
...
The raw LinearSVC output is a signed margin score that does not directly represent an observed mortality rate. Calibrated probabilities are obtained using \textbf{Platt scaling} \cite{platt1999}: \texttt{SVC(probability=True)} internally fits a logistic regression $\sigma(A\,f + B)$ on the signed margin $f_i$ via 5-fold cross-validation:
...
\node[pipe] (p6) at (0,-7.5)
  {\textbf{6.}~Platt scaling --- $\hat{p} = \sigma(A\cdot f_{\text{SVM}}+B)$ via \texttt{SVC(probability=True)}};
...
\begin{itemize}
  ...
  \item \texttt{probability} — Platt-scaled probability of 1-year mortality (0–1), output of \texttt{SVC(probability=True)}
...
\draw[msgret] (14,-7.3)  -- node[msglbl]{7. $\hat{p}$ $\in$ [0,1] — single predict\_proba() (Platt-scaled by \texttt{SVC(probability=True)})} (7,-7.3);
