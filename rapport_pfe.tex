...
%--- Title Formatting (platform color scheme) ---
\usepackage{titlesec}
...
\titleformat{\section}
  ...
  {0pt}
  {}
  [\vspace{-2pt}{\color{primary!25}\rule{\textwidth}{0.6pt}}]
\titlespacing*{\section}{0pt}{14pt}{6pt}

\titleformat{\subsection}
  {\normalfont\large\bfseries\color{primarydark}}
  {{\color{accent}\thesubsection}\enspace}
  {0pt}
  {}
...
% UEMF logo + school name
\IfFileExists{UEMF.png}{%
  \includegraphics[width=0.72\linewidth,keepaspectratio]{UEMF.png}%
}{\framebox[5cm][c]{\textbf{G UEMF}}}\\[0.8cm]

{\LARGE\bfseries\color{teal} EUROMED UNIVERSITY OF FES}\\[0.25cm]
{\large\color{teal}\textit{Euromed School of Digital Engineering and Artificial Intelligence}}\\[1cm]

{\large\bfseries Academic year 2025--2026}\\[0.25cm]
{\large\bfseries State engineering cycle in big data analytics}\\[0.8cm]

\rule{0.55\linewidth}{1pt}\\[0.4cm]
{\LARGE End of studies Project}\\[0.4cm]
\rule{0.55\linewidth}{1pt}\\[0.8cm]

\fbox{\parbox{0.82\linewidth}{\centering\vspace{4pt}
  Design and Development of an Intelligent Platform for Clinical Risk Analysis and
  Prediction in Nephrology Using Big Data and Machine Learning Techniques.%
\vspace{4pt}}}\\[1.2cm]

\begin{tabular}{p{0.42\linewidth}p{0.42\linewidth}}
  \textbf{Presented by :} & \textbf{Supervised by:}\\[0.4cm]
  BEN-ALI Nisrine         & Prof. Sara Bakkali\\[0.8cm]
                          & \textbf{Defended in} \dotfill\\[0.8cm]
  \textbf{Jury Members:}  & \textbf{Company supervisor:}\\[0.4cm]
                          & Mr Zakaria Alami Merrouni\\[0.8cm]
                          & \textbf{Hosting Company:}\\[0.2cm]
                          & HAIM-FMPDF\\
\end{tabular}
...
Garc\'ia-Montemayor et al. \cite{garcia2021} & Spanish HD ($n=1{,}336$) & RF & 0.73 & 1-year mortality \\
...
\begin{itemize}
  ...
  \item \textbf{Charlson Comorbidity Index (CCI)}: Validated in dialysis populations \cite{beddhu2000charlson}; higher comorbidity burden is independently associated with reduced survival, with a strong dose-response relationship observed across CCI categories \cite{beddhu2000charlson}.
  ...
  \item \textbf{Interaction features}: Non-linear combinations (e.g., age $\times$ CCI, albumin $\times$ hospitalizations) capture patient fragility beyond what individual variables convey \cite{zhu2021, albumin_dialysis_2018}.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Khan Score} \cite{khan2008score}: Comorbidity-based survival model developed for renal replacement therapy patients; simple scoring system with modest discrimination in external validations.
...
\begin{enumerate}
  \item \textbf{External validity}: Most published models are trained on large Western or East-Asian registries — e.g., Sheng et al.\ \cite{sheng2020} ($n = 5{,}351$, China) or Garc\'ia-Montemayor et al.\ \cite{garcia2021} ($n = 1{,}336$, Spain) — with demographic and etiological profiles significantly different from Moroccan patients (higher proportion of young diabetic patients, lower transplantation rates, different socioeconomic determinants).
...
These interaction features were constructed based on clinical evidence from the nephrology literature regarding combined risk factors in hemodialysis mortality \cite{beddhu2000charlson,albumin_dialysis_2018,kdigo2015,waikar2011sodium,dopps2022}.
...
\begin{enumerate}
  ...
  \item \textbf{Risk zone classifier}: Faible ($\hat{p}<10\%$), Modérée ($10\%\leq\hat{p}<29\%$), Élevée ($\hat{p}\geq29\%$) — seuils dérivés par ROC/Youden bootstrappé
\end{enumerate}
...
\medskip
\noindent\textbf{Clinical validation.} Both thresholds were reviewed and confirmed by the nephrology team at CHU Hassan II de Fès, who validated their utility for daily triage decisions. The resulting zones produce a strongly monotone mortality gradient on the HD-478 cohort (3.8\% $\to$ 15.6\% $\to$ 46.5\%), confirming their clinical discriminative power.
...
\begin{itemize}
  \item \texttt{risk\_level} — categorical classification: \textit{Faible} ($<$10\,\%), \textit{Modéré} (10–29\,\%), or \textit{Élevé} ($\geq$29\,\%)
...
\paragraph{Carnet numérique (global floating notepad).}
All authenticated pages include a \textbf{floating action button} (bottom-right corner) that opens a persistent personal notepad called \textit{Carnet numérique}. The notepad allows clinicians to write private reminders, observations, or annotations that are auto-saved at regular intervals and stored in their user profile (\texttt{personal\_notes} field). The panel displays the last save time and offers an \textbf{Effacer} button to clear the content.

\begin{figure}[H]
\centering
\IfFileExists{carnet.png}{\includegraphics[width=0.40\textwidth]{carnet.png}}{\framebox[0.40\textwidth][c]{\small[Screenshot: Carnet numérique floating notepad]}}
\caption{Carnet numérique --- persistent personal notepad accessible from any authenticated page via a floating action button}
\label{fig:carnet}
\end{figure}
...
\begin{figure}[H]
\centering
\IfFileExists{demande_soumission.png}{\includegraphics[width=0.85\textwidth]{demande_soumission.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Import submission request view]}}
...
\begin{figure}[H]
\centering
\IfFileExists{apercu_validation.png}{\includegraphics[width=0.85\textwidth]{apercu_validation.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Chef de service validation preview modal]}}
\caption{Import validation preview --- chef de service reviews file metadata, row/column count, quality score and first rows before approving or rejecting}
...
\begin{figure}[H]
\centering
\IfFileExists{data_mgmt_table.png}{\includegraphics[width=0.85\textwidth]{data_mgmt_table.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Data Management --- patient table with column mapping]}}
...
\begin{figure}[H]
\centering
\IfFileExists{data_mgmt_filter.png}{\includegraphics[width=0.85\textwidth]{data_mgmt_filter.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Data Management filter panel]}}
...
\begin{figure}[H]
\centering
\IfFileExists{data_mgmt_edit.png}{\includegraphics[width=0.85\textwidth]{data_mgmt_edit.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Patient record edit form]}}
...
\begin{figure}[H]
\centering
\IfFileExists{data_mgmt_dynamic_cols.png}{\includegraphics[width=0.85\textwidth]{data_mgmt_dynamic_cols.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Dynamic columns visible in patient table]}}
\caption{Dynamic column support --- institution-specific variables (stored in \texttt{extra\_data} JSONB) displayed as regular table columns}
\label{fig:data_mgmt_dynamic_cols}
\end{figure}

\begin{figure}[H]
\centering
\IfFileExists{data_mgmt_dynamic_form.png}{\includegraphics[width=0.72\textwidth]{data_mgmt_dynamic_form.png}}{\framebox[0.72\textwidth][c]{\small[Screenshot: Dynamic columns management panel]}}
...
\begin{figure}[H]
\centering
\IfFileExists{analysis_charts.png}{\includegraphics[width=0.85\textwidth]{analysis_charts.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Analysis tab --- statistical charts]}}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_model_dashboard.png}{\includegraphics[width=0.85\textwidth]{ai_model_dashboard.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: AI Model analysis dashboard]}}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_patients_list.png}{\includegraphics[width=0.85\textwidth]{ai_patients_list.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Patient list in AI Model --- with Predict button]}}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_prediction_tab.png}{\includegraphics[width=0.85\textwidth]{ai_prediction_tab.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Prediction tab --- risk gauge, probability, top-5 factors, recommendation]}}
...
\begin{itemize}
  \item The calibrated probability and risk zone badge (Faible / Modérée / Élevée)
...
\begin{figure}[H]
\centering
\IfFileExists{monitor_board.png}{\includegraphics[width=0.85\textwidth]{monitor_board.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Patient Monitoring Board]}}
...
\begin{figure}[H]
\centering
\IfFileExists{profile_page.png}{\includegraphics[width=0.85\textwidth]{profile_page.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Profile page --- user info, password change, language selector]}}
...
\begin{figure}[H]
\centering
\IfFileExists{chart_monthly.png}{\includegraphics[width=0.85\textwidth]{chart_monthly.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Monthly dialysis trend line chart]}}
...
\begin{figure}[H]
\centering
\IfFileExists{chart_sex.png}{\includegraphics[width=0.85\textwidth]{chart_sex.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Sex distribution doughnut chart]}}
...
\begin{figure}[H]
\centering
\IfFileExists{chart_age.png}{\includegraphics[width=0.85\textwidth]{chart_age.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Age distribution grouped bar chart]}}
...
\begin{figure}[H]
\centering
\IfFileExists{chart_complications.png}{\includegraphics[width=0.85\textwidth]{chart_complications.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Complication types horizontal bar chart]}}
...
\begin{figure}[H]
\centering
\IfFileExists{chart_etiology.png}{\includegraphics[width=0.85\textwidth]{chart_etiology.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: CKD etiology grouped bar chart]}}
...
\begin{figure}[H]
\centering
\IfFileExists{chart_comorbidities.png}{\includegraphics[width=0.85\textwidth]{chart_comorbidities.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Comorbidity combinations horizontal bar chart]}}
...
\begin{figure}[H]
\centering
\IfFileExists{prediction_display.png}{\includegraphics[width=0.85\textwidth]{prediction_display.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Full prediction result panel]}}
...
\begin{itemize}
  \item A \textbf{risk gauge} (SVG arc, green/orange/red) whose needle position corresponds to the Platt-scaled probability $\hat{p}$
  \item The \textbf{mortality probability} shown numerically (e.g., "47.1\% estimated 1-year mortality risk")
...
\begin{figure}[H]
\centering
\IfFileExists{ai_model_dashboard.png}{\includegraphics[width=0.85\textwidth]{ai_model_dashboard.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: AI model dashboard with cohort-level prediction charts]}}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_chart_zones.png}{\includegraphics[width=0.75\textwidth]{ai_chart_zones.png}}{\framebox[0.75\textwidth][c]{\small[Screenshot: AI risk zone distribution chart]}}
\caption{Cohort risk zone distribution --- proportion of patients classified as Faible, Modérée, and Élevée risk}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_chart_outcome.png}{\includegraphics[width=0.80\textwidth]{ai_chart_outcome.png}}{\framebox[0.80\textwidth][c]{\small[Screenshot: AI predicted outcome chart]}}
\caption{Predicted one-year mortality outcome distribution across the hemodialysis cohort}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_chart_kaplan_meier.png}{\includegraphics[width=0.82\textwidth]{ai_chart_kaplan_meier.png}}{\framebox[0.82\textwidth][c]{\small[Screenshot: Kaplan-Meier survival curves by risk zone]}}
\caption{Kaplan-Meier survival curves stratified by predicted risk zone --- Faible (green), Modérée (orange), Élevée (red)}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_chart_factors.png}{\includegraphics[width=0.82\textwidth]{ai_chart_factors.png}}{\framebox[0.82\textwidth][c]{\small[Screenshot: Top risk factors bar chart]}}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_chart_risk_profile.png}{\includegraphics[width=0.72\textwidth]{ai_chart_risk_profile.png}}{\framebox[0.72\textwidth][c]{\small[Screenshot: Risk profile radar chart]}}
\caption{Multi-dimensional clinical risk profile --- radar chart of normalized key indicators for Faible, Modérée, and Élevée zones}
...
\begin{figure}[H]
\centering
\IfFileExists{ai_chart_boxplots_zones.png}{\includegraphics[width=0.85\textwidth]{ai_chart_boxplots_zones.png}}{\framebox[0.85\textwidth][c]{\small[Screenshot: Box plots of biological indicators by risk zone]}}
\caption{Serum albumin distribution by risk zone --- interquartile range (Q1--Q3) for Zone Faible, Modérée, and Élevée}
\label{fig:ai_chart_boxplots_zones}
\end{figure}

This chart displays the interquartile distribution (Q1--Q3) of serum albumin across the three risk zones. The progressive decrease in albumin levels from Zone Faible ($\sim$40 g/L) to Zone Élevée ($\sim$30 g/L) confirms hypoalbuminemia as a key biological marker of elevated mortality risk in hemodialysis patients.
...
\draw[msgret] (14,-8.0)  -- node[msglbl]{8. $\hat{p}$ (Platt-scaled probability via CalibratedClassifierCV)} (7,-8.0);
\draw[msgfwd] (7,-8.7)   -- node[msglbl]{9. $\hat{p}<10\%$$\rightarrow$Faible,\ $10\%{\leq}\hat{p}<29\%$$\rightarrow$Modéré,\ $\hat{p}{\geq}29\%$$\rightarrow$Élevé} (7,-8.7);
...
The three-zone stratification — Faible ($\hat{p}<10\%$), Modérée ($10\%\leq\hat{p}<29\%$), Élevée ($\hat{p}\geq29\%$) — with thresholds derived by ROC/Youden bootstrap analysis and validated by the nephrology team of CHU Hassan II, was confirmed clinically meaningful for daily triage and resource allocation.
...
\bibitem{garcia2021}
Garc\'ia-Montemayor V, Soriano S, Bu\'endia P, et al. (2021).
\newblock Predicting mortality in hemodialysis patients using machine learning analysis.
\newblock \textit{Clinical Kidney Journal}, 14(7):1874--1882.
\newblock PMID: 34221370. DOI: 10.1093/ckj/sfab086.
...
\bibitem{dopps2022}
Robinson BM, Zhang J, Morgenstern H, Bradbury BD, Ng LJ, McCullough KP, Gillespie BW, Hakim R, Rayner H, Fort J, Akizawa T, Tentori F, Pisoni RL (2014).
...
\newblock PMID: 23978267. DOI: 10.1038/ki.2013.327.
