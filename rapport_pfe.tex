...
\newtcolorbox{resultbox}[1]{
  colback=medgreen!8,colframe=medgreen!60!black,
  fonttitle=\bfseries,title=#1,breakable
}
\newtcolorbox{notebox}{
...
\node[lbox,fill=accent!10,draw=accent,below=0.35cm of L1] (L2) {
  ...
  \textit{\color{textsec}\footnotesize 45+ endpoints \textbullet\ JWT (access 8h / refresh 7d) \textbullet\ 4 permission levels}
};
...
\node[lbox,fill=primarydark!10,draw=primarydark,below=0.35cm of L4] (L5) {
  ...
  PostgreSQL 16 (JSONB clinical sections) \textbullet\ Redis 7 (cache \& broker)\\
  \textit{\color{textsec}\footnotesize 11 JSONB sections per patient \textbullet\ GIN indexes \textbullet\ audit logs}
};
...
\node[ent, below=0pt of PH,  anchor=north] (PB) {%
  ...
  icc\_charlson : CharField\\11 sections JSONField\\
...
\texttt{password} & VARCHAR(128) & NO & Bcrypt-hashed password \\
...
\texttt{details} & TEXT & JSON-encoded action details \\
...
Clinical constraints are enforced at the serializer level:
\begin{itemize}
  \item Albumin: $5 \leq \text{albumin} \leq 60$ g/L (\cite{albumin_dialysis_2018})
  \item Sodium: $100 \leq \text{Na} \leq 180$ mmol/L (\cite{kdigo2015})
  \item Age: $0 \leq \text{age} \leq 120$ years
  \item Kt/V: $0 \leq \text{Kt/V} \leq 5$ (\cite{kdigo2015})
\end{itemize}

Values outside these ranges are flagged for review, not rejected outright, since extreme values may be clinically valid.

These validation constraints serve as a first-line data quality gate, flagging entries that are biologically implausible. Rather than rejecting such values outright (which could discard legitimate extreme values in critically ill patients), the platform flags them for clinician review. This soft-constraint approach balances data quality enforcement with clinical flexibility, consistent with best practices for electronic health record validation \cite{kdigo2015}.

\subsection{Audit Logs}

Every create, update, delete, import, and export action generates an \texttt{AuditLog} entry with: actor (user ID), action type, affected entity, diff of changed fields, IP address, and timestamp. Logs are automatically purged after 15 days via a Celery Beat periodic task.
...
The platform deploys \textbf{Qwen2.5:14B} \cite{llm_clinical_nlp_2023} locally via Ollama as the data quality analysis model. Qwen2.5:14B was chosen for: (1) its strong reasoning on structured tabular data and medical terminology; (2) local deployment — patient data never leaves the hospital infrastructure; (3) its 14-billion parameter capacity for contextualizing clinical values, detecting unit errors, and generating justified corrections.

The LLM processing workflow:
\begin{enumerate}
  \item The uploaded file is parsed into a DataFrame and each row is submitted to the LLM with its column names and values
  \item The LLM identifies anomalies (impossible values, wrong units, heterogeneous formats) and proposes corrections with a medical justification for each
  \item Corrections are stored in the \texttt{PreprocessValidationRequest} session and presented to the clinician for review
  \item The clinician accepts, overrides, or rejects each correction individually
  \item The validated version is submitted to the Chef de service for final approval before database integration
\end{enumerate}

Before calling the LLM, the platform runs an \textbf{intelligent routing step} implemented in \texttt{estimate\_route()} and called by the Celery task on every import. The motivation is resource efficiency: running Qwen2.5:14B on every uploaded file would impose a systematic 1--2 minute latency even for small, clean datasets where LLM analysis brings no value. The routing function evaluates three indicators of dataset complexity — percentage of missing values, number of outliers in biological columns, and presence of critical clinical columns (albumin, creatinine, dialysis variables) with high missingness — and selects one of three processing modes:

\begin{itemize}
  \item \textbf{Deterministic only}: clean file ($\leq$50 rows, 0\% missing, no outliers) — Python rules only, no LLM, sub-second processing
  \item \textbf{Balanced}: moderate complexity — lighter Qwen2.5:7b model, faster inference
  \item \textbf{Advanced}: complex medical dataset ($>$300 rows, $\geq$10\% missing, or clinical complexity score $\geq$6) — full Qwen2.5:14B
\end{itemize}
...
For biological columns, the platform also runs a \textbf{dedicated biological correction pass} that matches each column against the LOINC reference database and the medical knowledge base to detect unit errors and outliers independently of the LLM general analysis.
...
\rowcolor{medgreen!20}\textbf{SVM (Linear)} & \textbf{0.826} & \textbf{0.489} & \textbf{0.567} & \textbf{0.127} & \textbf{\ding{51}} \\
...
The linear SVM achieved the highest AUC-ROC (0.826), PR-AUC (0.489), and F1-score (0.567) while maintaining competitive Brier Score (0.127). Despite the ensemble's close performance, SVM was preferred for three clinical reasons: (1) linear decision boundary provides direct feature weight interpretation; (2) strong regularization prevents overfitting on a 478-patient cohort; (3) single-model structure facilitates regulatory compliance and audit.
\end{resultbox}
...
Figure~\ref{fig:roc_curves} shows the ROC curves for all nine evaluated models.

\begin{figure}[H]
\centering
\IfFileExists{1_courbes_ROC.png}{%
  \includegraphics[width=\textwidth,keepaspectratio]{1_courbes_ROC.png}
}{%
  \framebox[0.85\textwidth][c]{\small[ROC Curves --- 1\_courbes\_ROC.png]}%
}
\caption{ROC curves for all nine ML models evaluated on the HD-478 cohort (stratified 10-fold CV). The SVM linear model achieves the highest AUC-ROC = 0.826.}
\label{fig:roc_curves}
\end{figure}

Figure~\ref{fig:auc_gap} presents the train-test AUC gap analysis for each model, used to detect overfitting.
...
\begin{axis}[
  ...
  xlabel={Train-Test AUC Gap},
  xmin=-0.06, xmax=0.16,
  xmajorgrids=true,
  ytick={1,2,3,4,5,6},
  yticklabels={LASSO (L1), Random Forest, XGBoost, Log.\ Regression, LDA, SVM Lin\'{e}aire},
  ...
  title={Train-Test AUC Gap — Overfitting Analysis (HD-478 Cohort)},
  title style={font=\bfseries\small}
]
\addplot[fill=teal!60, draw=teal!80!black] coordinates {
  (0.0784,1) (-0.0133,2) (0.0959,3) (0.0588,4) (0.0475,5) (0.0467,6)
};
% Threshold line at 0.1
\draw[dashed,red!70!black,thick] (axis cs:0.1,0.4) -- (axis cs:0.1,6.6)
  node[above,font=\scriptsize\bfseries,red!70!black]{seuil = 0.1};
\end{axis}
\end{tikzpicture}
\caption{Train-test AUC gap for each evaluated model. A gap $< 0.1$ (dashed red line) indicates no significant overfitting. The SVM linear model achieves the lowest gap (0.047), confirming strong generalization on the 478-patient cohort.}
\label{fig:auc_gap}
\end{figure}

Figure~\ref{fig:radar} shows a radar plot comparing the five main performance metrics across all models.

\begin{figure}[H]
\centering
\IfFileExists{3_radar_performance.png}{%
  \includegraphics[width=0.90\textwidth,keepaspectratio]{3_radar_performance.png}
}{%
  \framebox[0.75\textwidth][c]{\small[Radar --- 3\_radar\_performance.png]}%
}
\caption{Multi-metric radar chart comparing AUC-ROC, PR-AUC, F1, Brier Score, and Recall across all evaluated models.}
\label{fig:radar}
\end{figure}

Figure~\ref{fig:calibration} shows the calibration curves, evaluating how well predicted probabilities match observed mortality rates.

\begin{figure}[H]
\centering
\IfFileExists{4_calibration.png}{%
  \includegraphics[width=\textwidth,keepaspectratio]{4_calibration.png}
}{%
  \framebox[0.80\textwidth][c]{\small[Calibration --- 4\_calibration.png]}%
}
\caption{Calibration curves (reliability diagrams) before and after isotonic calibration for the SVM model. After calibration, predicted probabilities closely track the observed mortality fraction.}
\label{fig:calibration}
\end{figure}

Figure~\ref{fig:confusion} presents the confusion matrices for the top three models.

\begin{figure}[H]
\centering
\IfFileExists{5_matrices_confusion.png}{%
  \includegraphics[width=\textwidth,keepaspectratio]{5_matrices_confusion.png}
}{%
  \framebox[0.90\textwidth][c]{\small[Confusion --- 5\_matrices\_confusion.png]}%
}
\caption{Confusion matrices for the top three models (SVM, XGBoost, Gradient Boosting) on the test set. True positives (correctly identified deaths) are shown in the lower-right cell.}
\label{fig:confusion}
\end{figure}

Figure~\ref{fig:pr_curve} shows the precision-recall curves for all models.

\begin{figure}[H]
\centering
\IfFileExists{7_precision_rappel.png}{%
  \includegraphics[width=\textwidth,keepaspectratio]{7_precision_rappel.png}
}{%
  \framebox[0.85\textwidth][c]{\small[PR Curve --- 7\_precision\_rappel.png]}%
}
\caption{Precision-Recall curves for all evaluated models. PR-AUC is the primary metric used for model selection given the class imbalance (80.1\% vs 19.9\%).}
\label{fig:pr_curve}
\end{figure}

Figure~\ref{fig:boxplot_folds} shows the cross-validation fold variance analysis.

\begin{figure}[H]
\centering
\IfFileExists{8_boxplot_folds.png}{%
  \includegraphics[width=\textwidth,keepaspectratio]{8_boxplot_folds.png}
}{%
  \framebox[0.85\textwidth][c]{\small[Boxplot --- 8\_boxplot\_folds.png]}%
}
\caption{Boxplot of AUC-ROC scores across 10 stratified cross-validation folds for each model, illustrating variance and stability of model performance.}
...
\text{zone} =
\begin{cases}
\textbf{Faible}   & \hat{p}_{\text{cal}} < 0.10 \\
\textbf{Modérée}  & 0.10 \leq \hat{p}_{\text{cal}} < 0.40 \\
\textbf{Élevée}   & \hat{p}_{\text{cal}} \geq 0.40
\end{cases}
\label{eq:risk_zones}
\end{equation}
...
\begin{table}[H]
\centering
\caption{Risk zones — clinical thresholds (TRIPOD/DOPPS/KDIGO), HD-478 cohort}
\label{tab:risk_zones}
\begin{tabular}{lcccc}
\toprule
\textbf{Zone} & \textbf{Seuil} & \textbf{Mortalité observée} & \textbf{Risque relatif} & \textbf{Action clinique} \\
\midrule
\cellcolor{green!20}\textbf{Faible}   & $\hat{p} < 10\%$             & 3.8\%  & $\times 1.0$ & Suivi standard \\
\cellcolor{orange!20}\textbf{Modérée} & $10\% \leq \hat{p} < 40\%$  & 22.9\% & $\times 3.6$ & Surveillance renforcée \\
\cellcolor{red!20}\textbf{Élevée}    & $\hat{p} \geq 40\%$           & 49.0\% & $\times 7.8$ & Prise en charge urgente \\
\bottomrule
\end{tabular}
\end{table}

Gradient 3.8\% $\to$ 22.9\% $\to$ 49.0\% — risque relatif Élevée/Faible : $\mathbf{\times 12.9}$.
...
\begin{tikzpicture}[
  node distance=0.8cm,
  box/.style={rectangle,draw,fill=blue!10,minimum width=7cm,minimum height=0.8cm,align=center,rounded corners=3pt,font=\small},
  decision/.style={diamond,draw,fill=orange!20,minimum width=3.5cm,align=center,font=\small},
  arrow/.style={-{Stealth},thick}
]
\node[box] (patient) {Patient selected in UI};
\node[box,below=of patient] (extract) {Feature Extraction (32 features)\\via \texttt{extract\_features(patient)}};
\node[box,below=of extract] (impute) {KNNImputer(k=3) — handle missing values};
\node[box,below=of impute] (transform) {PowerTransformer(Yeo-Johnson) + StandardScaler};
\node[box,below=of transform] (svm) {SVC(C=0.01, linear) — raw probability $p_{\text{raw}}$};
\node[box,below=of svm] (isotonic) {iso\_calibrator.joblib $\rightarrow$ $\hat{p}_{\text{cal}} = \text{Iso}(p_{\text{raw}})$};
\node[decision,below=1cm of isotonic] (zone) {Risk zone?};
\node[box,below left=1cm and -1.5cm of zone,fill=green!15] (low) {Faible\\$\hat{p}_{\text{cal}} < 10\%$};
\node[box,below=1.2cm of zone,fill=orange!15] (mod) {Modérée\\$10\% \leq \hat{p} < 40\%$};
\node[box,below right=1cm and -1.5cm of zone,fill=red!15] (high) {Élevée\\$\hat{p}_{\text{cal}} \geq 40\%$};
\node[box,below=3cm of zone,fill=purple!10] (output) {Output: zone + probability + top-5 factors + recommendation};

\draw[arrow] (patient) -- (extract);
\draw[arrow] (extract) -- (impute);
\draw[arrow] (impute) -- (transform);
\draw[arrow] (transform) -- (svm);
\draw[arrow] (svm) -- (isotonic);
\draw[arrow] (isotonic) -- (zone);
\draw[arrow] (zone) -- (low);
\draw[arrow] (zone) -- (mod);
\draw[arrow] (zone) -- (high);
\draw[arrow] (low) |- (output);
\draw[arrow] (mod) -- (output);
\draw[arrow] (high) |- (output);
\end{tikzpicture}
\caption{Complete prediction workflow for a single patient}
...
\textbf{AUC-ROC} measures the probability that the model ranks a randomly chosen positive example higher than a randomly chosen negative example \cite{hanley1982auc}:
\begin{equation}
\text{AUC} = P(\hat{p}_{y=1} > \hat{p}_{y=0})
\label{eq:auc}
\end{equation}

\textbf{Precision-Recall AUC (PR-AUC)} is preferred under class imbalance, as it focuses on the positive (minority) class \cite{collins_tripod_2015}:
\begin{align}
\text{Precision} &= \frac{TP}{TP + FP} \label{eq:precision}\\
\text{Recall} &= \frac{TP}{TP + FN} \label{eq:recall}\\
\text{F1} &= \frac{2 \cdot \text{Precision} \cdot \text{Recall}}{\text{Precision} + \text{Recall}} \label{eq:f1}
\end{align}

\subsection{Clinical Relevance: Threshold Validation}

The isotonic calibration ensures that predicted probabilities correspond to actual observed mortality rates. The three clinical thresholds ($T_1=10\%$, $T_2=40\%$) are grounded in evidence-based nephrology guidelines (TRIPOD/DOPPS/KDIGO) and were confirmed operationally meaningful by the CHU Hassan II nephrology team for daily triage.

\section{Model Generalization and Statistical Validation}\label{sec:generalization}

\begin{itemize}
  \item \textbf{Train-test AUC gap}: 0.047 ($\ll$ 0.1 threshold) — no evidence of overfitting
  \item \textbf{Permutation test}: 1000 permutations of the target variable; p-value = 0.020 — the model is statistically significant
  \item \textbf{Nested cross-validation AUC}: 0.8241 — confirms generalization in a fully independent evaluation
\end{itemize}

\section{Limitations of the ML Layer}\label{sec:ml_limits}

\subsection{Data Bias}
The HD-478 cohort is retrospective and single-center. Selection bias may arise from excluding patients lost to follow-up. The 478-sample size limits statistical power for subgroup analyses (e.g., per-etiology mortality). External validation on a second center cohort is needed.

\subsection{Model Generalization}
The model was optimized for the CHU Hassan II patient population (predominantly Moroccan, with specific etiological distribution). Direct deployment in a European or sub-Saharan African HD center may require recalibration.
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  class/.style={rectangle,draw,thick,fill=white,drop shadow},
  title/.style={fill=darkblue!80,text=white,font=\bfseries\small,
                minimum width=4.2cm,minimum height=0.6cm,align=center},
  attr/.style={fill=white,font=\footnotesize,minimum width=4.2cm,
               minimum height=0.45cm,align=left,inner sep=4pt},
  rel/.style={-{Stealth},thick},
  comp/.style={diamond,draw,fill=darkblue!20,minimum size=0.3cm},
  node distance=2.5cm
]

%--- Role ---
\node[title]  (r_t) at (0,0)       {Role};
\node[attr,below=0pt of r_t,anchor=north]  (r1) {+ id : Integer (PK)};
\node[attr,below=0pt of r1,anchor=north]   (r2) {+ name : CharField (unique)};
\node[attr,below=0pt of r2,anchor=north]   (r3) {+ label : CharField};
\draw[class] (r_t.north west) rectangle (r3.south east);

%--- Utilisateur ---
\node[title,right=3.5cm of r_t]  (u_t) {Utilisateur};
\node[attr,below=0pt of u_t,anchor=north]  (u1) {+ id : Integer (PK)};
\node[attr,below=0pt of u1,anchor=north]   (u2) {+ email : EmailField (unique)};
\node[attr,below=0pt of u2,anchor=north]   (u3) {+ nom, prenom : CharField};
\node[attr,below=0pt of u3,anchor=north]   (u4) {+ telephone : CharField};
\node[attr,below=0pt of u4,anchor=north]   (u5) {+ role\_id : FK $\rightarrow$ Role};
\node[attr,below=0pt of u5,anchor=north]   (u6) {+ personal\_notes : TextField};
\node[attr,below=0pt of u6,anchor=north]   (u7) {+ is\_active : Boolean};
\node[attr,below=0pt of u7,anchor=north,fill=blue!8]  (um1) {+ login() : JWT};
\node[attr,below=0pt of um1,anchor=north,fill=blue!8] (um2) {+ change\_password()};
\draw[class] (u_t.north west) rectangle (um2.south east);

%--- Patient ---
\node[title,right=3.5cm of u_t]  (p_t) {Patient};
\node[attr,below=0pt of p_t,anchor=north]  (p1) {+ id : Integer (PK)};
\node[attr,below=0pt of p1,anchor=north]   (p2) {+ id\_patient : CharField};
\node[attr,below=0pt of p2,anchor=north]   (p3) {+ icc\_charlson : Integer};
\node[attr,below=0pt of p3,anchor=north]   (p4) {+ demographie\_data : JSONB};
\node[attr,below=0pt of p4,anchor=north]   (p5) {+ biologie\_data : JSONB};
\node[attr,below=0pt of p5,anchor=north]   (p6) {+ dialyse\_data : JSONB};
\node[attr,below=0pt of p6,anchor=north]   (p7) {+ [8 more JSONB sections]};
\node[attr,below=0pt of p7,anchor=north]   (p8) {+ extra\_data : JSONB};
\node[attr,below=0pt of p8,anchor=north]   (p9) {+ utilisateur\_saisie : FK};
\node[attr,below=0pt of p9,anchor=north,fill=blue!8]  (pm1) {+ get\_flat\_view()};
\node[attr,below=0pt of pm1,anchor=north,fill=blue!8] (pm2) {+ extract\_features() : list};
\draw[class] (p_t.north west) rectangle (pm2.south east);

%--- AuditLog ---
\node[title,below=2.8cm of r3,anchor=north]  (a_t) {AuditLog};
\node[attr,below=0pt of a_t,anchor=north]  (a1) {+ id : Integer (PK)};
\node[attr,below=0pt of a1,anchor=north]   (a2) {+ action : CharField};
\node[attr,below=0pt of a2,anchor=north]   (a3) {+ entite, entite\_id};
\node[attr,below=0pt of a3,anchor=north]   (a4) {+ details : JSONB};
\node[attr,below=0pt of a4,anchor=north]   (a5) {+ adresse\_ip : IPField};
\node[attr,below=0pt of a5,anchor=north]   (a6) {+ timestamp : DateTime};
\node[attr,below=0pt of a6,anchor=north]   (a7) {+ utilisateur\_id : FK};
\draw[class] (a_t.north west) rectangle (a7.south east);

%--- PreprocessValidationRequest ---
\node[title,right=3.5cm of a_t]  (pr_t) {PreprocessValidationRequest};
\node[attr,below=0pt of pr_t,anchor=north]  (pr1) {+ id : Integer (PK)};
\node[attr,below=0pt of pr1,anchor=north]   (pr2) {+ session\_id : UUIDField};
\node[attr,below=0pt of pr2,anchor=north]   (pr3) {+ fichier\_source : CharField};
\node[attr,below=0pt of pr3,anchor=north]   (pr4) {+ statut : CharField};
\node[attr,below=0pt of pr4,anchor=north]   (pr5) {+ soumis\_par\_id : FK};
\node[attr,below=0pt of pr5,anchor=north]   (pr6) {+ valide\_par\_id : FK};
\node[attr,below=0pt of pr6,anchor=north]   (pr7) {+ commentaire : TextField};
\draw[class] (pr_t.north west) rectangle (pr7.south east);

%--- Relationships ---
% Role 1 --< Utilisateur
\draw[rel,{Diamond[open]}-{Stealth}]
  (r_t.east) -- node[above,font=\tiny]{1} ++(0.6,0)
  node[above,font=\tiny]{} -- node[above,font=\tiny]{*} (u_t.west);
\node[font=\tiny,above] at ($(r_t.east)!0.5!(u_t.west)$) {has};

% Utilisateur 1 --< Patient
\draw[rel,{Diamond[open]}-{Stealth}]
  (u_t.east) -- node[above,font=\tiny]{1\quad *} (p_t.west);
\node[font=\tiny,above] at ($(u_t.east)!0.5!(p_t.west)$) {saisie\_par};

% Utilisateur 1 --< AuditLog
\draw[rel]
  (u1.west) -- ++(-0.3,0) |- node[left,font=\tiny,pos=0.7]{1..* audit} (a_t.north);

% Utilisateur 1 --< PreprocessValidationRequest
\draw[rel]
  (u5.east) ++(4.2,0) -- ++(0.5,0) |-
  node[right,font=\tiny,pos=0.3]{submits} (pr5.east);
...
\bottomrule
\endlastfoot
POST & \texttt{/api/auth/login/} & JWT login $\rightarrow$ access + refresh token \\
POST & \texttt{/api/auth/refresh/} & Refresh expired access token \\
GET & \texttt{/api/auth/users/} & List all users (admin) \\
POST & \texttt{/api/auth/users/} & Create user account \\
PATCH & \texttt{/api/auth/users/\{id\}/} & Update user profile \\
DELETE & \texttt{/api/auth/users/\{id\}/} & Delete user (super admin) \\
GET & \texttt{/api/patients/} & List patients (paginated, filterable) \\
POST & \texttt{/api/patients/} & Create new patient record \\
GET & \texttt{/api/patients/\{id\}/} & Full patient record \\
PATCH & \texttt{/api/patients/\{id\}/} & Update patient data \\
POST & \texttt{/api/patients/import/} & Import Excel/CSV file \\
GET & \texttt{/api/patients/export/} & Export all patients as .xlsx \\
GET & \texttt{/api/patients/flat/} & Flat view for analytics charts \\
POST & \texttt{/api/patients/preprocess/analyze/} & Start async LLM preprocessing \\
GET & \texttt{/api/patients/preprocess/\{sid\}/status/} & Preprocessing session status \\
GET & \texttt{/api/patients/preprocess/\{sid\}/rows/} & LLM-suggested corrections per row \\
PATCH & \texttt{/api/patients/preprocess/\{sid\}/cell-corrections/} & Override LLM correction \\
POST & \texttt{/api/patients/preprocess/\{sid\}/integrate/} & Finalize data integration \\
POST & \texttt{/api/predictions/train/} & Retrain SVM on current dataset \\
POST & \texttt{/api/predictions/predict-patient/} & Predict mortality for 1 patient \\
GET & \texttt{/api/predictions/metrics/} & Model performance metrics \\
GET & \texttt{/api/audit/} & Paginated audit log (admin) \\
DELETE & \texttt{/api/audit/cleanup-old/} & Manual log cleanup \\
\end{longtable}
...
Authentication follows the OAuth2-compatible JWT flow:
\begin{enumerate}
  \item Client sends \texttt{POST /api/auth/login/} with email and password
  \item Server validates credentials and returns \texttt{\{access: <jwt>, refresh: <jwt>\}}
  \item Client attaches \texttt{Authorization: Bearer <access>} to subsequent requests
  \item Access token expires after 8 hours; refresh token after 7 days
  \item Axios interceptor automatically refreshes expired tokens transparently
\end{enumerate}

The JWT payload encodes: \texttt{user\_id}, \texttt{role}, \texttt{exp} (expiration timestamp), \texttt{iat} (issued at). The server verifies the signature using \texttt{HS256} with a 256-bit secret key.

\section{API Design}\label{sec:api}

\subsection{Prediction Response Structure}

The prediction API response provides clinicians with a complete, interpretable summary. The \texttt{POST /api/predictions/predict-patient/} endpoint returns: \texttt{risk\_zone} (Faible/Modérée/Élevée), \texttt{calibrated\_probability} (0–1), \texttt{raw\_probability}, \texttt{relative\_risk}, \texttt{top\_factors} (list of 5 SVM feature weights with direction), \texttt{recommendation} (clinical text), \texttt{features\_used} (32), and \texttt{model\_version}.
...
\subsection{Application Sidebar and Navigation}

\begin{figure}[H]
\centering
\framebox[0.3\textwidth][c]{\small[Screenshot: Application Sidebar]}
\caption{AI NéphroCare navigation sidebar --- main interface entry points}
\label{fig:sidebar}
\end{figure}
...
\begin{itemize}
  \item \textbf{Platform logo and branding} at the top
  \item \textbf{Navigation links} to all main sections: Dashboard, Patient Management (Preprocessing, Data Management, Analysis), AI Model, Monitoring
  \item \textbf{User role indicator} showing the current user's role (Admin, Chef de service, Résident, Professeur)
  \item \textbf{Logout button} at the bottom
\end{itemize}
...
\begin{description}
  \item[\textbf{Dashboard} (\texttt{/dashboard})] The dashboard interface varies by user role:

\textbf{Administrator dashboard} displays \textbf{7 KPI cards}:
\begin{itemize}
  \item Total active accounts
  \item Total inactive accounts
  \item Number of administrator accounts
  \item Number of Chef de service accounts
  \item Number of Résident accounts
  \item Number of Professeur accounts
  \item Total number of existing roles
\end{itemize}
Additionally, the administrator dashboard includes a full user management table with create, edit, and delete actions.

\textbf{Chef de service dashboard} displays \textbf{6 KPI cards} (excluding admin-related metrics):
\begin{itemize}
  \item Total active accounts
  \item Total inactive accounts
  \item Number of Chef de service accounts
...
\textbf{Important}: Any modification of user account information (email, role, password, or status) requires the acting user to enter their own account password for authentication confirmation before the change is saved. This double-authentication mechanism prevents unauthorized modifications.

\textbf{Account creation}: To add a new account, the administrator must fill in the following mandatory fields: email address, telephone number, last name (\textit{nom}), first name (\textit{prénom}), role (Admin / Chef de service / Résident / Professeur), account status (active/inactive), and an initial password.

\begin{figure}[H]
\centering
\framebox[0.85\textwidth][c]{\small[Screenshot: Admin Dashboard with 7 KPI cards]}
\caption{Administrator dashboard --- 7 KPI cards and user management table}
...
\begin{figure}[H]
\centering
\framebox[0.85\textwidth][c]{\small[Screenshot: Preprocessing Interface --- file upload and LLM analysis]}
\caption{Preprocessing interface --- file upload, LLM analysis with Qwen 14B, and correction review workflow}
\label{fig:preproc_ui}
\end{figure}

The preprocessing tab allows authorized users to upload Excel or CSV patient data files and subject them to an AI-driven data quality analysis. The platform uses the \textbf{Qwen2.5:14B} language model (deployed locally via Ollama), a state-of-the-art large language model specifically chosen for its:
\begin{itemize}
  \item \textbf{Strong reasoning capacity} on structured tabular data and medical terminology
  \item \textbf{Local deployment capability}: runs on-premises via Ollama, ensuring patient data never leaves the hospital infrastructure
  \item \textbf{14 billion parameters}: sufficient capacity to contextualize clinical values, detect unit errors, and suggest corrections with medical justification
\end{itemize}

The preprocessing workflow proceeds as follows:
\begin{enumerate}
  \item The user uploads an Excel or CSV file containing patient records
  \item The platform parses the file and displays a data profile (column names, types, null rates)
  \item The user clicks \textbf{"Launch LLM Analysis"} to start the asynchronous Qwen2.5:14B analysis via Celery
  \item A \textbf{progress indicator} shows the real-time analysis progress
  \item Once complete, a \textbf{correction review table} displays each row with LLM-suggested corrections, including a medical justification for each change
  \item The user can \textbf{accept, modify, or reject} each correction individually
  \item Once satisfied, the user submits the validated data to the \textbf{Chef de service} for final approval
  \item Upon approval, the data is \textbf{integrated into the platform database}
\end{enumerate}

The interface displays the following windows and panels:
\begin{itemize}
  \item \textbf{File upload panel}: drag-and-drop zone with file format validation
  \item \textbf{Data profile panel}: table showing column statistics
  \item \textbf{LLM analysis progress window}: real-time progress bar with current row being processed
  \item \textbf{Correction review table}: row-by-row LLM suggestions with justifications
  \item \textbf{Validation buttons}: Accept All, Reject All, and per-cell override
  \item \textbf{Submit for approval} button (sends to Chef de service)
\end{itemize}

Once preprocessing is complete and the version is selected, the system transitions directly to the \textbf{Data Management tab}, where the values from the selected file version are mapped to the fixed columns of the platform's standardized patient table.

\begin{figure}[H]
\centering
\framebox[0.85\textwidth][c]{\small[Screenshot: LLM correction review table with suggestions and justifications]}
\caption{LLM correction review interface --- Qwen2.5:14B suggestions with medical justifications}
\label{fig:llm_corrections}
\end{figure}
...
\node[actorbox] (ML) at (14,0)   {SVM Pipeline\\(.joblib)};
...
\draw[msgfwd] (7,-9.4)   -- node[msglbl]{10. iso\_cal $\rightarrow$ $\hat{p}$; zones: $<10\%$$\rightarrow$Faible, $10{-}40\%$$\rightarrow$Modérée, $\geq40\%$$\rightarrow$Élevée} (7,-9.4);
...
\draw[msgret] (7,-11.5)  -- node[msglbl]{13. \{risk\_zone, p\_calib, top\_5\_factors, recommendation\}} (3.5,-11.5);
...
\node[draw,fill=green!10,font=\tiny,align=left,inner sep=3pt] at (11.5,-10.3)
  {Faible: $\hat{p} < 10\%$\\Modérée: $10\% \leq \hat{p} < 40\%$\\Élevée: $\hat{p} \geq 40\%$\\(TRIPOD/DOPPS/KDIGO)};
